...
export class OpsAuthService {
  ...
  static async bootstrapOwner() {
    ...
    if (process.env.NODE_ENV === "production" && process.env.OPS_BOOTSTRAP_ENABLE !== "true") {
      console.warn("OPS_BOOTSTRAP_SKIPPED: OPS_BOOTSTRAP_ENABLE not true in production");
      return;
    }
