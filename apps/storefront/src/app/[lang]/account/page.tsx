...
export default function AccountPage({ params }: { params: Promise<{ lang: string }> }) {
    ...
    const handleLogout = () => {
        ...
        document.cookie = 'vayva_storefront_token=; path=/; expires=Thu, 01 Jan 1970 00:00:01 GMT;';
        router.push(`/${lang}/account/login`);
    };
                ...
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="glass-panel p-6 rounded-2xl">
                            <h2 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4">{t.planTitle}</h2>
                            <div className="flex items-center gap-3">
                                <div className="text-xl font-bold text-gray-900">{t.planDesc}</div>
                        ...
                        {/* Quick Actions */}
                        <div className="glass-panel p-6 rounded-2xl">
                            <h2 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4">{t.quickLinks}</h2>
                            <div className="grid grid-cols-1 gap-3">
                                <Link href={`/${lang}/menu`} className="group flex items-center gap-3 p-3 rounded-xl hover:bg-black/5 transition-all">
                                    ...
                                    <span className="font-bold text-gray-700 group-hover:text-black">{t.menu}</span>
                                </Link>
                                <Link href={`/${lang}/past-deliveries`} className="group flex items-center gap-3 p-3 rounded-xl hover:bg-black/5 transition-colors">
                                    ...
                                    <span className="font-bold text-gray-700 group-hover:text-black">{t.history}</span>
                                </Link>
                                <Link href={`/${lang}/favorites`} className="group flex items-center gap-3 p-3 rounded-xl hover:bg-black/5 transition-colors">
                                    ...
                                    <span className="font-bold text-gray-700 group-hover:text-black">{t.gift}</span>
                                </Link>
                            </div>
                        </div>
                    </div>
