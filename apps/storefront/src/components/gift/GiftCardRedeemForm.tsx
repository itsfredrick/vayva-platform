...
export function GiftCardRedeemForm({ lang }: GiftCardRedeemFormProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (redemption.success) {
            setResult({ success: true, message: `${t.success} +${redemption.amount}₺` });
            setCode('');
        } else {
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 p-6 md:p-8 shadow-sm h-full flex flex-col justify-center">
            ...
            {result && (
                <div className={`mb-6 p-4 rounded-xl flex items-center gap-3 text-sm font-bold animate-fade-in-up ${result.success ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'
                    }`}>
