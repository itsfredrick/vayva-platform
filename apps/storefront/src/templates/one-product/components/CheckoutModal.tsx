...
export const CheckoutModal = ({ isOpen, onClose, product, qty, upsellProduct }: CheckoutModalProps) => {
    ...
    const total = (product.price * qty) + (upsellAdded && upsellProduct ? upsellProduct.price : 0);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3002'}/api/orders/create`, {
                method: 'POST',
        ...
        return (
            <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
                <div className="bg-white rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl">
                    ...
                    <p className="text-gray-500 mb-6">We've sent a receipt to <b>{email}</b>. We'll start processing your delivery to <b>{city}</b> shortly.</p>
                </div>
            </div>
        );
    }
        ...
        <div className="fixed inset-0 z-[100] flex justify-end bg-black/50 backdrop-blur-sm animate-in slide-in-from-right-10 duration-300">
            <div className="w-full max-w-md bg-white h-full shadow-2xl flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    {/* Order Summary */}
                    <div className="mb-8">
                        <h3 className="text-xs font-bold text-gray-400 uppercase mb-4">Order Summary</h3>
                        <div className="flex gap-4 mb-4">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex justify-between mt-1">
                                    ...
                                    <span className="font-bold text-sm">₦{(product.price * qty).toLocaleString()}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Upsell */}
                        {upsellProduct && (
                            <div className={`border-2 rounded-xl p-4 transition-all ${upsellAdded ? 'border-green-500 bg-green-50' : 'border-dashed border-gray-300'}`}>
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="flex-1">
                                        <div className="flex justify-between items-start">
                                            <span className="font-bold text-sm text-gray-900 block mb-1">One-time Offer: Add {upsellProduct.name}?</span>
                                            <span className="font-bold text-sm text-gray-900"> +₦{upsellProduct.price.toLocaleString()}</span>
                                        </div>
                                        <p className="text-xs text-gray-500 mb-2">{upsellProduct.description}</p>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    <form id="checkout-form" onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Contact Info</label>
                            <input
                                type="email"
                ...
                <div className="p-4 border-t border-gray-100 bg-gray-50">
                    ...
                    <button form="checkout-form" type="submit" disabled={loading} className="w-full bg-[#111827] text-white font-bold py-4 rounded-xl shadow-lg hover:bg-black transition-all disabled:opacity-50">
