...
export const BulkHeader = ({ storeName, rfqCount = 0, onOpenRFQ }: BulkHeaderProps) => {
    return (
        <header className="bg-white border-b border-gray-200 sticky top-0 z-50">
            ...
            <div className="max-w-7xl mx-auto px-6 h-20 flex items-center gap-8">
                {/* Brand */}
                <Link href="/" className="flex items-center gap-3 text-[#0F172A] min-w-max">
                    <div className="bg-[#0F172A] text-white p-2 rounded">
                        <Truck size={24} />
                    </div>
                    <div>
                        <h1 className="font-black text-xl leading-none tracking-tight">{storeName?.toUpperCase() || 'BULKTRADE'}</h1>
                        <p className="text-[10px] font-bold text-gray-400 tracking-widest uppercase">Wholesale Direct</p>
                    </div>
                </Link>
