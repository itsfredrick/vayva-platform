...
export const RFQDrawer = ({ isOpen, onClose, items, onRemoveItem, onUpdateQty, onSubmit }: RFQDrawerProps) => {
            ...
            <div className="w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
                ...
                <div className="p-6 border-b border-gray-200 bg-[#0F172A] text-white flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs text-gray-400">{items.length} items ready for pricing review</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    {items.length === 0 ? (
                        <div className="text-center text-gray-400 py-12">
                            ...
                            <button onClick={onClose} className="mt-4 text-blue-600 font-bold hover:underline">Browse Catalog</button>
                        </div>
                    ) : (
                            ...
                            return (
                                <div key={product.id} className="flex gap-4 border-b border-gray-100 pb-6">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-gray-500 mb-2">SKU: {product.id.toUpperCase()}</p>
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <input
                                                    type="number"
                                                    value={qty}
                                                    onChange={(e) => onUpdateQty(product.id, parseInt(e.target.value) || 0)}
                                        ...
                                        {!isMoqMet && (
                                            <p className="text-[10px] text-red-600 mt-1 font-bold">Below MOQ ({moq} units)</p>
                                        )}
                                    </div>
                                </div>
                            );
                    ...
                    {items.length > 0 && (
                        <div>
                            <label className="block text-sm font-bold text-gray-700 mb-2">Additional Notes / Delivery Terms</label>
                            <textarea
                                value={notes}
                ...
                {items.length > 0 && (
                    <div className="p-6 border-t border-gray-200 bg-gray-50">
                        <div className="flex items-center justify-between mb-4 text-sm">
                            <span className="text-gray-500">Estimated Total (Indicative)</span>
                            <span className="font-bold text-gray-900">~ ₦{items.reduce((acc, item) => acc + (item.product.price * item.qty), 0).toLocaleString()}</span>
                        </div>
                        <button
                            onClick={handleSubmit}
                            disabled={submitting || items.some(i => i.qty < (i.product.wholesaleDetails?.moq || 1))}
                        ...
                        <p className="text-center text-[10px] text-gray-400 mt-3">
                            This is not a final invoice. You will receive a formal quote within 24 hours.
                        </p>
                    </div>
                )}
            </div>
