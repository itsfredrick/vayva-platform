...
export const ItemModal = ({ item, onClose, onAddToCart }: ItemModalProps) => {
    ...
    useEffect(() => {
        ...
        Object.values(selectedModifiers).forEach((val: any) => {
            ...
            if (Array.isArray(val)) { // Multi-select
    ...
    const handleToggle = (modId: string, option: any) => {
        ...
        // Mocking 'addon' as single select for simplicity in this demo unless specified.
        ...
        // Just mocking the state update for the UI visual:
        ...
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center pointer-events-none">
            ...
            <div className="bg-white w-full max-w-md sm:rounded-t-2xl sm:rounded-b-2xl h-[85vh] flex flex-col pointer-events-auto animate-in slide-in-from-bottom duration-300">
                ...
                <div className="relative h-48 bg-gray-100 flex-shrink-0">
                    ...
                    />
                    <button onClick={onClose} className="absolute top-4 right-4 bg-white/90 p-2 rounded-full text-gray-900 shadow-sm">
                        <X size={20} />
                    </button>
                </div>
                    ...
                    {item.modifiers?.map((mod) => (
                        <div key={mod.id} className="mb-6">
                            <div className="flex justify-between items-center mb-3">
                                ...
                                {mod.type === 'choice' && <span className="text-xs bg-gray-100 px-2 py-0.5 rounded text-gray-500">Required</span>}
                            </div>

                            <div className="space-y-2">
                                {mod.options.map((opt, idx) => (
                                    <label key={idx} className="flex items-center justify-between p-3 border border-gray-100 rounded-lg has-[:checked]:border-red-600 has-[:checked]:bg-red-50 cursor-pointer">
                                        ...
                                        {opt.price > 0 && <span className="text-xs font-semibold text-gray-500">+₦{opt.price}</span>}
                                    </label>
                                ))}
                            </div>
                        </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 pb-8 space-y-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] rounded-b-2xl">
                    <div className="flex items-center justify-center gap-6 mb-2">
                        <button onClick={() => setQty(Math.max(1, qty - 1))} className="p-2 bg-gray-100 rounded-full hover:bg-gray-200"><Minus size={20} /></button>
                        <span className="text-xl font-extrabold w-8 text-center">{qty}</span>
                        <button onClick={() => setQty(qty + 1)} className="p-2 bg-gray-100 rounded-full hover:bg-gray-200"><Plus size={20} /></button>
                    </div>

                    <button
                        onClick={() => onAddToCart({ ...item, qty, modifiers: selectedModifiers }, total)}
