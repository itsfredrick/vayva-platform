            ...
            <div className="max-w-5xl mx-auto flex flex-col md:flex-row gap-12 items-center">
                ...
                <div className="flex-1 space-y-6">
                    <div className="flex items-center gap-2">
                        <span className="bg-blue-100 text-blue-700 text-xs font-bold px-2 py-0.5 rounded uppercase">{course.courseDetails?.level}</span>
                    ...
                    <div className="flex items-center gap-3 py-2 border-t border-b border-gray-200">
                        ...
                        />
                        <div>
                            <p className="text-sm font-bold text-gray-900">{course.courseDetails?.instructor.name}</p>
                            <p className="text-xs text-gray-500">{course.courseDetails?.instructor.title}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 pt-2">
                        ...
                        >
                            {course.price === 0 ? 'Start Learning for Free' : `Enroll for ₦${course.price.toLocaleString()}`}
                        </button>
                    </div>
                </div>
                ...
                <div className="w-full md:w-[400px] bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 group cursor-pointer relative">
                    ...
                    <div className="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition-colors flex items-center justify-center">
                        <div className="w-16 h-16 bg-white/90 rounded-full flex items-center justify-center text-blue-600 backdrop-blur-sm shadow-lg group-hover:scale-110 transition-transform">
                            <PlayCircle size={32} fill="currentColor" className="text-blue-600" />
                        </div>
                    </div>
                    <div className="p-4">
                        <h3 className="font-bold text-gray-900 mb-1">Preview this course</h3>
                        <p className="text-xs text-gray-500">Watch the first lesson for free.</p>
                    </div>
                </div>
