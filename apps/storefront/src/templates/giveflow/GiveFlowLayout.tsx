...
export const GiveFlowLayout = ({ store, products }: GiveFlowLayoutProps) => {
    ...
    const [activeCampaignForDonation, setActiveCampaignForDonation] = useState<PublicProduct | null>(null);
    ...
    return (
        <div className="min-h-screen bg-[#F9FAFB] font-sans text-gray-900">
            ...
            <main>
                ...
                <section className="py-16 bg-gray-50 border-t border-gray-200">
                    <div className="max-w-6xl mx-auto px-6">
                        <h2 className="text-2xl font-bold text-gray-900 mb-8">More causes to support</h2>
                        ...
                        <div className="grid md:grid-cols-3 gap-8">
                            {otherCampaigns.map(campaign => {
                                ...
                                return (
                                    <div key={campaign.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow flex flex-col">
                                        <div className="h-48 relative">
                                            <img src={campaign.images?.[0]} className="w-full h-full object-cover" />
                                        ...
                                        <div className="p-6 flex-1 flex flex-col">
                                            <h3 className="font-bold text-lg text-gray-900 mb-2 leading-tight">{campaign.name}</h3>
                                            <p className="text-sm text-gray-500 mb-4 line-clamp-2">{campaign.description}</p>

                                            <div className="mt-auto">
                                                <div className="w-full bg-gray-100 rounded-full h-2 mb-2">
                                                    <div className="bg-[#16A34A] h-2 rounded-full" style={{ width: `${percentage}%` }}></div>
                                                </div>
                                                <div className="flex justify-between text-xs font-bold text-gray-600 mb-4">
                                                    <span>₦{details.raisedAmount.toLocaleString()} raised</span>
            ...
            <footer className="bg-white py-12 border-t border-gray-200 text-center">
                <p className="text-gray-400 text-sm">&copy; {new Date().getFullYear()} {store.name}. Giving makes the world better.</p>
            </footer>
            ...
            {activeCampaignForDonation && (
                <DonationOptions
                    ...
                    isRecurringAvailable={activeCampaignForDonation.donationDetails?.isRecurringAvailable}
                />
            )}
        </div>
    );
