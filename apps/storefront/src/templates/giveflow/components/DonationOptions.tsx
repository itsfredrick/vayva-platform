...
export const DonationOptions = ({ isOpen, onClose, campaignTitle, onDonate, isRecurringAvailable = true }: DonationOptionsProps) => {
        ...
        return (
            <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                <div className="bg-white rounded-2xl p-8 max-w-sm w-full text-center animate-in zoom-in-95">
                    ...
                    <p className="text-gray-500 mb-6">Your donation of <span className="font-bold text-gray-900">₦{amount.toLocaleString()}</span> has been processed securely.</p>
                    <p className="text-xs text-gray-400">A receipt has been sent to your email.</p>
                </div>
            </div>
        );
    }
        ...
        <div className="fixed inset-0 z-[100] flex justify-end bg-black/50 backdrop-blur-sm">
            <div className="w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
                ...
                <div className="p-6 flex-1 overflow-y-auto">
                    ...
                    <label className="block text-sm font-bold text-gray-700 mb-4">Choose an amount</label>
                    ...
                    <div className="mb-8">
                        <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Custom Amount</label>
                    ...
                    <div className="bg-gray-50 p-4 rounded-xl mb-6">
                        <div className="flex items-start gap-3">
                            <div className="mt-1 text-[#16A34A]"><Heart size={18} fill="currentColor" /></div>
                            <div>
                                <h4 className="font-bold text-sm text-gray-900">Your Impact</h4>
                                <p className="text-xs text-gray-500 mt-1">
                                    ₦{amount.toLocaleString()} could help provide {Math.floor(amount / 2500)} meals for families in need.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="p-6 border-t border-gray-100">
                    ...
                    >
                        {isRecurring ? `Donate ₦${amount.toLocaleString()} Monthly` : `Donate ₦${amount.toLocaleString()}`}
                    </button>
