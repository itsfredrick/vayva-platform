...
export const TicketlyLayout = ({ store, products }: TicketlyLayoutProps) => {
    ...
    const [checkoutData, setCheckoutData] = useState<{ total: number; count: number } | null>(null);
    const [successAttendee, setSuccessAttendee] = useState<{ name: string; email: string } | null>(null);
        ...
        <div className="min-h-screen bg-white font-sans text-gray-900">
                    ...
                    <div className="max-w-6xl mx-auto">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {products.slice(1).map(evt => (
                                <div key={evt.id} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                                    <div className="h-48 bg-gray-200">
                                        <img src={evt.images?.[0]} className="w-full h-full object-cover" />
                                    </div>
                                    <div className="p-6">
                                        <div className="text-purple-600 text-xs font-bold uppercase tracking-wider mb-2">
                                            {new Date(evt.eventDetails?.date || '').toLocaleDateString()}
                                        </div>
                                        <h3 className="font-bold text-lg mb-2">{evt.name}</h3>
                                        <p className="text-sm text-gray-500 mb-4">{evt.eventDetails?.venue}</p>
            ...
            <footer className="bg-gray-900 text-white py-12 px-6">
                <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center opacity-50 text-sm">
                    <p>&copy; {new Date().getFullYear()} {store.name}. Powered by Ticketly.</p>
