...
export const FileDetail = ({ product, onClose, onPurchase }: FileDetailProps) => {
    return (
                ...
                <div className="flex-1 p-8 overflow-y-auto bg-[#1F2937]">
                    ...
                    <div className="mb-6">
                        <div className="flex items-center gap-2 mb-2">
                            <span className="bg-indigo-500/20 text-indigo-400 text-xs font-bold px-2 py-0.5 rounded uppercase">{product.category}</span>
                            {product.licenseType === 'extended' && <span className="bg-purple-500/20 text-purple-400 text-xs font-bold px-2 py-0.5 rounded uppercase">Extended License</span>}
                        </div>
                        <h2 className="text-3xl font-extrabold text-white mb-2">{product.name}</h2>
                        <div className="flex items-center gap-4 text-sm text-gray-400">
                            <span className="flex items-center gap-1"><Star size={14} className="text-yellow-500" /> 4.9 (120)</span>
                            <span>•</span>
                    ...
                    <p className="text-gray-400 leading-relaxed mb-8">
                        {product.description}
                        <br /><br />
                        Unlock immediate access to this premium asset. Includes lifetime updates and support.
                    </p>

                    <div className="bg-[#111827] rounded-xl p-4 mb-8">
                        <h4 className="text-sm font-bold text-gray-300 mb-3 uppercase tracking-wider">File Specs</h4>
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div className="flex justify-between border-b border-gray-800 pb-2">
                                <span className="text-gray-500">Format</span>
                                <span className="text-white font-medium">{product.fileDetails?.fileType}</span>
                            </div>
                            <div className="flex justify-between border-b border-gray-800 pb-2">
                                <span className="text-gray-500">Size</span>
                                <span className="text-white font-medium">{product.fileDetails?.fileSize}</span>
                            </div>
                            <div className="flex justify-between border-b border-gray-800 pb-2">
                                <span className="text-gray-500">License</span>
                                <span className="text-white font-medium capitalize">{product.licenseType || 'Standard'}</span>
                            </div>
                        </div>
                    </div>
