...
const FileIcon = ({ type }: { type?: string }) => {
    if (type === 'PDF' || type === 'DOCX') return <FileText size={20} className="text-red-400" />;
    if (type === 'FIG' || type === 'ZIP') return <FileCode size={20} className="text-indigo-400" />;
    return <FileImage size={20} className="text-blue-400" />;
};

export const ProductGridDigital = ({ products, onSelect }: ProductGridDigitalProps) => {
    return (
