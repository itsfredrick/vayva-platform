...
export const StorefrontService = {
     ...
     * Create Order
     */
    createOrder: async (orderData: any): Promise<{ success: boolean; orderId?: string; paymentUrl?: string; error?: string }> => {
        try {
     ...
     */
    initializePayment: async (paymentData: { orderId: string; email: string; amount: number; callbackUrl: string }): Promise<any> => {
        try {
     ...
     * Get Order Status
     */
    getOrderStatus: async (orderId: string, phone?: string): Promise<PublicOrder | null> => {
        try {
            const response = await fetch(`${API_BASE}/orders/${orderId}${phone ? `?phone=${phone}` : ''}`, {
                cache: 'no-store'
            });
            if (response.ok) return await response.json();
            return null;
    ...
    getActiveFlashSale: async (storeId: string): Promise<any | null> => {
        try {
            const response = await fetch(`${API_BASE}/marketing/flash-sale?storeId=${storeId}`, {
