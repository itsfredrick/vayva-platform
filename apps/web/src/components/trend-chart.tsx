...
export function TrendChart({ data, color = '#46EC13', height = 200 }: { data: number[], color?: string, height?: number }) {
    // Determine bounds
    ...
    return (
        <div className="w-full relative" style={{ height: `${height}px` }}>
            <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full overflow-visible">
                ...
                {/* Area Fill */}
                <path d={`M0,100 L0,${100 - ((data[0] - min) / range) * 80} ${points.split(' ').map(p => 'L' + p).join(' ')} L100,100 Z`} fill={`url(#gradient-${color})`} />
