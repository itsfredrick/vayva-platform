...
export function MarketingHeader() {
    ...
    return (
        <header className="fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-[#142210]/80 backdrop-blur-md border-b border-white/5">
            <div className="max-w-[1440px] mx-auto px-4 lg:px-6 h-20 flex items-center justify-between">
                ...
                <div className="hidden lg:flex items-center gap-4">
                    <Link href="/signin" className="text-sm font-bold text-white hover:text-[#46EC13] transition-colors">
                        Sign in
                    </Link>
                    <Link href="/signup">
            ...
            <AnimatePresence>
                {isMenuOpen && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -20 }}
                        className="fixed inset-0 top-20 bg-[#142210] z-40 flex flex-col p-6 lg:hidden"
                    >
                        <nav className="flex flex-col gap-6 mt-4">
                            {NAV_LINKS.map((link) => (
                                <Link
                                    ...
                                    className="text-2xl font-bold text-white/80 hover:text-[#46EC13]"
                                >
                                    {link.label}
                                </Link>
                            ))}
                            <hr className="border-white/10 my-4" />
                            <Link href="/signin" onClick={() => setIsMenuOpen(false)} className="text-xl font-bold text-white">
                                Sign in
                            </Link>
