...
export function MarketingFooter() {
    return (
        <footer className="bg-[#0b141a] border-t border-white/5 pt-20 pb-10">
            <div className="max-w-[1440px] mx-auto px-4 lg:px-6">
                        ...
                        <div className="flex gap-4">
                            {['twitter', 'instagram', 'linkedin', 'facebook'].map((social) => (
                                <a
                                    key={social}
                                    href={`#${social}`}
                                    className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white/60 hover:text-[#46EC13] hover:bg-white/10 transition-all"
                                >
                            ...
                            <ul className="space-y-4">
                                {links.map((link) => (
                                    <li key={link.href}>
                                        <Link
                                            href={link.href}
                                            className="text-sm text-white/50 hover:text-[#46EC13] transition-colors"
                                        >
                                            {link.label}
                                        </Link>
                                    </li>
                ...
                <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex gap-6">
                        <Link href="/legal/privacy" className="text-xs text-white/30 hover:text-white">Privacy Policy</Link>
                        <Link href="/legal/terms" className="text-xs text-white/30 hover:text-white">Terms of Service</Link>
                    </div>
                </div>
            </div>
        </footer>
