...
export function isRoleAtLeast(userRole: string, requiredRole: AppRole): boolean {
...
export function requireRole(user: SessionUser | undefined, minimumRole: AppRole) {
...
export async function authorizeAction(user: SessionUser | undefined, minimumRole: AppRole): Promise<Response | null> {
    if (!user) {
