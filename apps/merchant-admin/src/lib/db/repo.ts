...
export class Repository {
  static products = {
    async findMany(ctx: TenantContext, args: any = {}) {
      return prisma.inventoryItem.findMany({
        // Using Inventory as proxy for Product in this example or Product model
        ...args,
        where: {
          ...args.where,
          InventoryLocation: {
  ...
  static orders = {
    async list(ctx: TenantContext) {
      ...
      // Testing return for compilation if Order model not strictly defined in this snippet
      return [];
    },
  };
