...
export type EscalationTrigger = 'PAYMENT_DISPUTE' | 'FRAUD_RISK' | 'SENTIMENT' | 'LOW_CONFIDENCE' | 'MANUAL_REQUEST' | 'BILLING_ERROR';
...
export class EscalationService {
    ...
    static async triggerHandoff(params: {
        ...
        try {
            ...
            logger.info('[EscalationService] Handoff triggered', { ticketId: ticket.id, ...params });
            return ticket;
    ...
    private static mapTriggerToPriority(trigger: EscalationTrigger): string {
        switch (trigger) {
            case 'PAYMENT_DISPUTE': return 'URGENT';
            case 'FRAUD_RISK': return 'URGENT';
            case 'BILLING_ERROR': return 'HIGH';
            case 'SENTIMENT': return 'HIGH';
    ...
    private static mapTriggerToCategory(trigger: EscalationTrigger): string {
        switch (trigger) {
            ...
            case 'FRAUD_RISK': return 'FRAUD';
            case 'BILLING_ERROR': return 'BILLING';
            case 'SENTIMENT': return 'OTHER';
    ...
    private static mapTriggerToType(trigger: EscalationTrigger): string {
        switch (trigger) {
            case 'BILLING_ERROR': return 'BILLING';
            case 'PAYMENT_DISPUTE': return 'BILLING';
            default: return 'GENERAL';
        }
    }
}
