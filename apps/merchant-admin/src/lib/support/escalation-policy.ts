...
export class EscalationPolicy {
    private static readonly SENSITIVE_KEYWORDS = {
        PAYMENT_DISPUTE: ['refund', 'chargeback', 'scam', 'fraud', 'stolen', 'unrecognized charge'],
        BILLING_ERROR: ['double charged', 'wrong amount', 'billing error', 'invoice mistake'],
     ...
     */
    static evaluate(query: string, confidence: number = 1.0, historyCount: number = 0): EscalationDecision {
        ...
        // 4. Manual Request
        if (lowerQuery.includes('talk to human') || lowerQuery.includes('support agent')) {
            return {
                shouldEscalate: true,
