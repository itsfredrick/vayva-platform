...
export async function getConsentStats(merchantId: string) {
  const [
    ...
    prisma.communication_consent.count({
      where: { merchantId, marketingOptIn: true },
    }),
    prisma.communication_consent.count({
      where: { merchantId, marketingOptIn: false },
    }),
    prisma.communication_consent.count({
      where: { merchantId, fullyBlocked: true },
    }),
    prisma.communication_consent.count({
      where: { merchantId, transactionalAllowed: false },
    }),
  ]);
...
export async function getReachableAudience(
  merchantId: string,
  type: MessageIntent,
) {
  ...
  if (type === MessageIntent.MARKETING) {
    ...
    const total = await prisma.communication_consent.count({
      where: { merchantId },
    });
