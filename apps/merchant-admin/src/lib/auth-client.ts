import { LoginRequest, SignupRequest, ForgotPasswordRequest, ResetPasswordRequest } from '@vayva/schemas';
...
class AuthClient {
    private async request<T>(endpoint: string, method: string, body: any): Promise<T> {
        ...
        const token = typeof window !== 'undefined' ? localStorage.getItem('vayva_token') : null;
        if (token) {
    ...
    async login(data: LoginRequest): Promise<AuthResponse> {
        const response = await this.request<AuthResponse>('/auth/login', 'POST', data);
        if (response.token) {
    ...
    async signup(data: SignupRequest): Promise<AuthResponse> {
        const response = await this.request<AuthResponse>('/auth/signup', 'POST', data);
        if (response.token) {
            this.setSession(response.token, response.user);
        }
        return response;
