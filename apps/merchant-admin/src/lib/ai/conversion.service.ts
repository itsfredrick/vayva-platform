...
export type PersuasionStrategy = 'BENEFIT_FRAME' | 'SOCIAL_PROOF' | 'RISK_REDUCTION' | 'NONE';
...
export class ConversionService {
    ...
    static classifyObjection(text: string): string | null {
        ...
        if (lower.includes('delivery') || lower.includes('shipping') || lower.includes('how long')) return 'DELIVERY';
        if (lower.includes('real') || lower.includes('fake') || lower.includes('trust')) return 'TRUST';
        return null;
    }
