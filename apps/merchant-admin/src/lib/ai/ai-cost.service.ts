...
export class AiCostService {
    ...
    static async estimateRequestCost(params: {
        ...
        const inputCostMicros = (params.inputTokens / 1000) * pricing.inputCostPer1K;
        const outputCostMicros = (params.outputTokens / 1000) * pricing.outputCostPer1K;
     ...
     */
    static async checkBudgetSafety(storeId: string): Promise<{ safe: boolean; reason?: string }> {
        // 1. Check Platform-wide budget
        const today = new Date();
        today.setHours(0, 0, 0, 0);

        const platformStat = await prisma.platformCostDaily.findUnique({ where: { date: today } });
        if (platformStat && platformStat.budgetUsedPercent >= 100) {
            return { safe: false, reason: 'Platform-wide AI budget reached. Service temporarily throttled.' };
        }
