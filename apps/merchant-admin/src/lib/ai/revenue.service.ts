...
export class RevenueService {
     ...
     */
    static async checkTrialEligibility(signal: AbuseSignal): Promise<{ allowed: boolean; reason?: string }> {
        ...
        if (pattern && pattern.signupCount >= 2) {
            return {
                allowed: false,
                reason: "Multiple trials detected from this device. To protect our community, we limit free trials to 2 per household/business every 90 days. Please upgrade to a Growth plan to continue."
            };
        }
