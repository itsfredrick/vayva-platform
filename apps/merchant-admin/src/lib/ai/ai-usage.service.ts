...
export class AiUsageService {
    ...
    static async logUsage(params: {
        ...
        try {
            ...
            await prisma.merchantAiSubscription.updateMany({
                where: { storeId, status: { in: ['TRIAL_ACTIVE', 'TRIAL_EXPIRED_GRACE', 'UPGRADED_ACTIVE'] } },
                data: {
            ...
            await prisma.aiUsageDaily.upsert({
                ...
                update: { requestsCount: { increment: 1 }, tokensCount: { increment: inputTokens + outputTokens } },
                create: { storeId, date: today, requestsCount: 1, tokensCount: inputTokens + outputTokens }
            });
    ...
    static async checkLimits(storeId: string): Promise<{
        ...
        if (!sub) return { allowed: false, reason: "No active AI subscription found.", usage: { messagesUsed: 0, messageLimit: 0, isOverLimit: true, status: 'NONE' } };

        // 1. Hard closure (Abuse or explicit closure)
        if (sub.status === 'SOFT_CLOSED' || sub.status === 'BLOCKED') {
            const reason = sub.status === 'BLOCKED' ? "Account blocked for abuse." : "AI Agent Closed. Upgrade to reactivate.";
            return { allowed: false, reason, usage: { messagesUsed: sub.monthMessagesUsed, messageLimit: 0, isOverLimit: true, status: sub.status } };
        }

        // 2. Trial Expiry (Time-based or Grace-based)
        const now = new Date();
        if (sub.status === 'TRIAL_EXPIRED_GRACE' || (sub.planKey === 'STARTER' && sub.trialExpiresAt < now)) {
            return { allowed: false, reason: "Trial period has ended. Please upgrade to a Growth plan.", usage: { messagesUsed: sub.monthMessagesUsed, messageLimit: 0, isOverLimit: true, status: sub.status } };
        }

        // 3. Calculate dynamic limit: Plan Limit + All Addon Packs
        const planLimit = sub.planKey === 'STARTER' ? 20 : sub.plan.monthlyRequestLimit;
        const addonMessages = sub.addonPurchases.reduce((sum, a) => sum + a.messagesAdded, 0);
