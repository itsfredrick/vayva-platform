 ...
 */
export async function detectStuckWithdrawals(storeId?: string): Promise<StuckWithdrawal[]> {
 ...
 */
export async function detectAgingPending(storeId?: string): Promise<StuckWithdrawal[]> {
 ...
 */
export async function detectStuckExports(merchantId?: string): Promise<StuckExport[]> {
    const now = new Date();
...
export async function getOpsMetrics(storeId?: string): Promise<OpsMetrics> {
    ...
    if (isP112Enabled) {
        ...
        try {
            ...
            integrations = {
                whatsapp: { status: 'NOT_INSTRUMENTED', lastSuccess: null, lastEvent: null },
                paystack: { status: 'NOT_INSTRUMENTED', lastSuccess: null, lastEvent: null },
                delivery: { status: 'NOT_INSTRUMENTED', lastSuccess: null, lastEvent: null }
            };
        }
    }
 ...
 */
export async function logStuckOperations(storeId: string, userId: string): Promise<void> {
    const stuck = await detectStuckWithdrawals(storeId);
    ...
    for (const withdrawal of stuck) {
        await logAuditEvent(
            storeId,
            userId,
            AuditEventType.OPERATION_STUCK,
            {
                type: 'withdrawal',
