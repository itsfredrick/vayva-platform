...
export async function syncOnboardingData(
  ...
  if (state.schemaVersion && state.schemaVersion !== EXPECTED_SCHEMA_VERSION) {
    console.warn(
      `[Sync][Drift Alarm] Schema version mismatch! Expected ${EXPECTED_SCHEMA_VERSION}, got ${state.schemaVersion}. Sync logic may be outdated.`,
    );
  }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.store.update({
        ...
        data: {
          ...
          settings: {
            domainPreference:
              state.storeDetails?.domainPreference || "subdomain",
            currency: state.payments?.currency || "NGN",
            payoutScheduleAcknowledged:
              state.payments?.payoutScheduleAcknowledged,
          },
      ...
      if (existingProfile) {
        ...
        await tx.storeProfile.create({
          data: {
            storeId,
            slug:
              state.storeDetails?.slug || `store-${storeId.substring(0, 8)}`,
      ...
      await tx.storeProfile.update({
        ...
        data: {
          ...
          pickupAddress: state.delivery?.pickupAddress || undefined,
          deliveryMethods:
      ...
      if (state.delivery?.policy) {
        const policyContent =
          state.delivery.policy === "pickup_only"
