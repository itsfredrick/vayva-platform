...
import { ACQUISITION_EVENTS, ACTIVATION_EVENTS, COMMERCE_EVENTS } from './events';
    ...
    static async getCheckoutFunnel(storeId: string, range: DateRange) {
        ...
        const [views, carts, checkouts, purchases] = await Promise.all([
            prisma.analyticsEvent.count({
                where: { storeId, action: COMMERCE_EVENTS.VIEW_PRODUCT, timestamp: { gte: range.from, lte: range.to } }
            }),
            prisma.analyticsEvent.count({
                where: { storeId, action: COMMERCE_EVENTS.ADD_TO_CART, timestamp: { gte: range.from, lte: range.to } }
            }),
            prisma.analyticsEvent.count({
                where: { storeId, action: COMMERCE_EVENTS.BEGIN_CHECKOUT, timestamp: { gte: range.from, lte: range.to } }
            }),
            prisma.analyticsEvent.count({
                where: { storeId, action: COMMERCE_EVENTS.PURCHASE_SUCCESS, timestamp: { gte: range.from, lte: range.to } }
            })
        ]);
