...
export function signReferralToken(partnerId: string, code: string, expiresInHours = 24 * 30): string {
...
export function verifyReferralToken(token: string): ReferralPayload | null {
    ...
    try {
        const payload: ReferralPayload = JSON.parse(Buffer.from(data, 'base64').toString());
