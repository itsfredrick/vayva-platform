...
export function authOtpVerification(data: {
  otp: string;
  first_name?: string;
}): string {
  return wrapEmail(
...
export function onboardingWelcome(data: {
  first_name: string;
  store_name: string;
...
export function billingReceipt(data: {
  store_name: string;
  amount: string;
  currency: string;
...
export function authAccountLocked(data: {
  unlock_link: string;
  reason?: string;
}): string {
  return wrapEmail(
...
// --- TCP-02: New Login ---
export function authNewLogin(data: {
  date: string;
  device: string;
  location: string;
  ip: string;
  lock_account_url?: string;
}): string {
  return wrapEmail(
    `
        ${Hero("hero_lock")}
...
export function authVerifyEmailLink(data: {
  verify_link: string;
  first_name?: string;
}): string {
  return wrapEmail(
...
export function authPasswordChanged(data: {
  date: string;
  support_url?: string;
}): string {
  return wrapEmail(
...
export function onboardingIncomplete(data: {
  first_name: string;
  next_step: string;
  dashboard_url?: string;
}): string {
...
export function billingInvoiceAvailable(data: {
  invoice_number: string;
  amount: string;
  due_date: string;
  download_url?: string;
...
export function billingPaymentFailed(data: {
  invoice_number: string;
  amount: string;
  retry_link: string;
  reason?: string;
...
export function billingSubscriptionStarted(data: {
  plan_name: string;
  amount: string;
  renewal_date: string;
}): string {
...
export function billingSubscriptionCancelled(data: {
  plan_name: string;
  end_date: string;
...
export function orderShippedCustomer(data: {
  order_id: string;
...
export function orderDeliveredCustomer(data: {
  order_id: string;
  support_link?: string;
}): string {
  return wrapEmail(
...
export function merchantNewOrder(data: {
  order_id: string;
  total: string;
  items: string;
  customer_name: string;
...
export function merchantPayoutProcessed(data: {
  amount: string;
  currency: string;
  bank_name: string;
...
export function billingSubscriptionExpiryReminder(data: {
  store_name: string;
  plan_name: string;
  expiry_date: string;
  billing_url: string;
...
export function systemMaintenance(data: {
  date: string;
