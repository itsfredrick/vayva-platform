...
// Name matching logic
export function calculateNameMatch(provided: { first: string; last: string }, verified: { first: string; last: string }): number {
    ...
    async verify(request: KycVerificationRequest): Promise<KycVerificationResult> {
        try {
            ...
            return {
                ...
                matchScore: response.data.allValidationPassed ? 100 : (selfieMatch?.confidenceLevel || 0),
                status: (response.success && response.data.status === 'found') ? 'VERIFIED' : 'FAILED',
                rawResponse: response.data,
                error: response.data.validations?.validationMessages || (response.data.status === 'not_found' ? 'Identity not found' : undefined)
            };
        } catch (error: any) {
            return {
                success: false,
...
export class KycService {
    ...
    constructor() {
        // Only real provider allowed - no mocks
        this.provider = new YouverifyProvider();
    }

    async verifyIdentity(storeId: string, request: KycVerificationRequest): Promise<KycVerificationResult> {
        ...
        try {
            ...
            if (result.status === 'VERIFIED') {
                ...
                await NotificationManager.trigger(storeId, 'KYC_FAILED', { reason: result.error || 'Identity mismatch' });
            }
        } catch (notifierErr) {
