...
export class LedgerService {
    ...
    static async recordTransaction(req: TransactionRequest) {
        return await prisma.$transaction(async (tx) => {
            ...
            if (walletImpact !== 0) {
                ...
                let wallet = await tx.wallet.findUnique({ where: { storeId: req.storeId } });
                if (!wallet) {
     ...
     */
    private static determineEntries(req: TransactionRequest): { account: string; direction: 'DEBIT' | 'CREDIT'; amount: number }[] {
        ...
        switch (type) {
            case WalletTransactionType.PAYMENT:
                ...
                entries.push({ account: 'wallet_available', direction: 'CREDIT', amount });
            ...
            case WalletTransactionType.PAYOUT:
                ...
                entries.push({ account: 'wallet_available', direction: 'DEBIT', amount });
                break;
