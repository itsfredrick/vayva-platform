...
export default function InsightsPage() {
    ...
    const segments = [
        { id: 'vip', title: 'VIP Customers', icon: Sparkles, color: 'text-yellow-500', bg: 'bg-yellow-500/10' },
        { id: 'loyal', title: 'Loyal Regulars', icon: Users, color: 'text-blue-500', bg: 'bg-blue-500/10' },
        { id: 'new', title: 'Recent & Promising', icon: TrendingUp, color: 'text-green-500', bg: 'bg-green-500/10' },
        { id: 'atRisk', title: 'At Risk', icon: AlertCircle, color: 'text-red-500', bg: 'bg-red-500/10' },
    ];
        ...
        <div className="space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-text-secondary">AI-driven segmentation to boost your retention.</p>
                </div>
            ...
            <div className="grid grid-cols-3 gap-4">
                <Card className="p-6 space-y-2">
                    <div className="text-sm text-text-secondary">Total Processed Revenue</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {segments.map(s => {
                    ...
                    return (
                        <Card key={s.id} className="p-6 flex flex-col justify-between h-48 hover:border-primary/40 transition-colors group">
                            <div className="flex justify-between items-start">
                                <div className={`p-3 rounded-xl ${s.bg} ${s.color}`}>
                                    <s.icon size={24} />
                                </div>
                                <Badge variant="default" className="bg-white/5">{segData.count} Customers</Badge>
                            </div>

                            <div>
                                <h3 className="text-lg font-bold">{s.title}</h3>
                                <p className="text-sm text-text-secondary">Est. Value: ₺{segData.revenue.toLocaleString()}</p>
                            </div>
