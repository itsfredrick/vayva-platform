...
export default function AuditLogPage() {
    ...
    const fetchLogs = async (reset = false) => {
        setLoading(true);
        try {
            const cursorParam = reset ? '' : (nextCursor ? `&limit=50&cursor=${nextCursor}` : '&limit=50');
    ...
    return (
        <div className="max-w-6xl mx-auto p-6 space-y-6">
            ...
            {/* Detail Drawer */}
            {selectedLog && (
                <div className="fixed inset-0 z-50 flex justify-end bg-black/20 backdrop-blur-sm" onClick={() => setSelectedLog(null)}>
                    <div
                    ...
                    >
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <button onClick={() => setSelectedLog(null)} className="text-gray-400 hover:text-gray-900">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        ...
                        <div className="space-y-6">
                            <div>
                                <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">Correlation ID</label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="mt-1 text-sm font-medium text-gray-900">{selectedLog.action}</p>
                                </div>
                                <div>
                                    <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">Actor</label>
                                    <p className="mt-1 text-sm font-medium text-gray-900">{selectedLog.actorLabel}</p>
                                </div>
                            </div>

                            {selectedLog.beforeState && (
                                <div>
                                    <label className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-2 block">Before State</label>
                            ...
                            {selectedLog.afterState && (
                                <div>
                                    <label className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-2 block">After State</label>
                                    <pre className="bg-gray-900 text-gray-100 p-3 rounded-lg text-xs overflow-x-auto font-mono">
