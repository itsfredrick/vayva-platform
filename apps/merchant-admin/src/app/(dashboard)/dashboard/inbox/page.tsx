        ...
        <div className="flex h-[calc(100vh-6rem)] -m-6 bg-white overflow-hidden">
            ...
            <div className="w-80 border-r border-gray-100 flex flex-col bg-gray-50/30">
                <div className="p-4 border-b border-gray-100">
                    ...
                    <input type="search" placeholder="Search..." className="w-full text-sm p-2 rounded-lg border border-gray-200 bg-white" />
                </div>
                    ...
                    {conversations.map(c => (
                        ...
                        >
                            <div className="flex justify-between items-start mb-1">
                                <span className={`font-bold text-sm ${c.unreadCount > 0 ? 'text-black' : 'text-gray-700'}`}>
                            ...
                            <p className="text-xs text-gray-500 truncate">{c.lastMessage?.textBody || 'No messages'}</p>

                            {/* Badges */}
                            <div className="flex gap-2 mt-2">
                                {c.slaStatus === 'overdue' && (
                                    <span className="bg-red-100 text-red-600 text-[10px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wide">Overdue</span>
                                )}
                                {c.unreadCount > 0 && (
                                    <span className="bg-blue-500 text-white text-[10px] px-1.5 py-0.5 rounded-full font-bold min-w-[20px] text-center">{c.unreadCount}</span>
                                )}
                            </div>
                        </div>
                    ))}
                                ...
                                {showQuickReplies && (
                                    ...
                                    >
                                        {quickReplies.map(qr => (
                                            ...
                                            >
                                                <span className="font-bold block text-xs">{qr.title}</span>
                                                <span className="text-gray-500 truncate block text-xs">{qr.content}</span>
                                            </button>
                                        ))}
                                        {quickReplies.length === 0 && <div className="p-2 text-xs text-gray-400">No templates found.</div>}
                                    </motion.div>
                                )}
                            </AnimatePresence>

                            {/* Input Area */}
                            <div className={`relative rounded-xl border ${showNotesInput ? 'border-yellow-300 bg-yellow-50' : 'border-gray-200 bg-white'}`}>
                                <textarea
                                    className="w-full p-3 bg-transparent outline-none text-sm resize-none rounded-xl"
                                    placeholder={showNotesInput ? "Add a private note for your team..." : "Type a message..."}
                                    rows={2}
                                    value={showNotesInput ? noteText : messageText}
                                    onChange={e => showNotesInput ? setNoteText(e.target.value) : setMessageText(e.target.value)}
                                />
                                <div className="flex justify-end p-2 border-t border-gray-100/50">
                                    <button
                                        onClick={handleSend} // Handle Note vs User Send logic
                                        disabled={sending}
                                        className={`px-4 py-1.5 rounded-lg text-sm font-bold text-white transition-opacity ${sending ? 'opacity-50' : ''} ${showNotesInput ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-green-600 hover:bg-green-700'
                                            }`}
                                    >
            ...
            {selectedId && (
                <div className="w-72 border-l border-gray-100 bg-white flex flex-col p-6 overflow-y-auto">
                    ...
                    {/* Orders Stub */}
                    <div className="mb-6">
                        <h4 className="text-xs font-bold uppercase text-gray-400 mb-3">Recent Orders</h4>
                        <div className="space-y-3">
                            <div className="p-3 border border-gray-100 rounded-lg hover:border-gray-200 cursor-pointer">
                                <div className="flex justify-between items-center mb-1">
                                    ...
                                    <span className="text-[10px] bg-yellow-50 text-yellow-700 px-1.5 rounded">Processing</span>
                                </div>
                    ...
                    {/* Tags */}
                    <div>
                        <h4 className="text-xs font-bold uppercase text-gray-400 mb-3">Tags & Labels</h4>
                        <div className="flex flex-wrap gap-2">
                            <button className="text-[10px] border border-dashed border-gray-300 px-2 py-1 rounded text-gray-400 hover:border-gray-400">+ Add Tag</button>
                        </div>
                    </div>
                </div>
            )}
