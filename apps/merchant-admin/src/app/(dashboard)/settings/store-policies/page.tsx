...
export default function StorePoliciesPage() {
    ...
    async function handleGenerate() {
        if (!confirm('Generate default policies? This will overwrite any existing drafts.')) return;

        setLoading(true);
        try {
            const res = await fetch('/api/merchant/policies/generate', { method: 'POST' });
            if (res.ok) {
                await loadPolicies();
    ...
    async function handlePublish() {
        if (!confirm('Publish this policy? It will be visible on your storefront.')) return;
        ...
        <div className="p-6 max-w-7xl mx-auto">
            ...
            <Card className="bg-amber-50 border-amber-200 p-4 mb-6">
                <p className="text-sm text-amber-900">
                    <strong>Note:</strong> These are templates. Review and customize them to match your business before publishing.
                </p>
            </Card>
