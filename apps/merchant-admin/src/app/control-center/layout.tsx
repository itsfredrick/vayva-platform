...
function ControlCenterNav() {
    ...
    const navItems = [
        { label: 'Returns Policy', href: '/control-center/policies?tab=returns', tab: 'returns', icon: RefreshCcw },
        { label: 'Shipping Policy', href: '/control-center/policies?tab=shipping', tab: 'shipping', icon: Truck },
        { label: 'Privacy Policy', href: '/control-center/policies?tab=privacy', tab: 'privacy', icon: Lock },
        { label: 'Terms of Service', href: '/control-center/policies?tab=terms', tab: 'terms', icon: FileText },
    ];

    return (
        <div className="sticky top-6">
            <h3 className="text-xs font-bold uppercase text-gray-400 mb-6 px-3">Store Policies</h3>
            <nav className="space-y-1">
                {navItems.map((item) => {
                    const isActive = pathname.includes('policies') && currentTab === item.tab;
                    return (
                        <Link
                            key={item.href}
                            href={item.href}
