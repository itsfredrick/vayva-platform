...
export async function POST(req: NextRequest) {
    ...
    if (!storeId) return new NextResponse('No active store', { status: 400 });
    ...
    try {
        ...
        const subscription = await prisma.merchantSubscription.findUnique({ where: { storeId } });
        const entitlement = {
