...
export async function GET(req: NextRequest) {
    try {
        ...
        for (const stat of customerStats) {
            ...
            const lastOrderDate = stat._max.createdAt ? new Date(stat._max.createdAt) : new Date();
            const firstOrderDate = stat._min.createdAt ? new Date(stat._min.createdAt) : new Date();

            totalRevenue += sales;
            totalOrders += count;

            const daysSinceLast = (now.getTime() - lastOrderDate.getTime()) / (1000 * 3600 * 24);
            const daysSinceFirst = (now.getTime() - firstOrderDate.getTime()) / (1000 * 3600 * 24);
