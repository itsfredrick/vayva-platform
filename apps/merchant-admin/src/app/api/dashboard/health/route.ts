...
export const GET = withRBAC(PERMISSIONS.COMMERCE_VIEW, async (session: any) => {
  try {
    ...
    const [totalOrders, paidOrders, refundedOrders, activeCustomers] =
      await Promise.all([
    ...
    if (totalOrders > 0) {
      ...
      if (successRate > 80)
        factors.push({
          id: "f1",
          text: "High Payment Success Rate",
          sentiment: "positive",
        });
      if (refundRate < 5)
        factors.push({
          id: "f2",
          text: "Low Refund Rate",
          sentiment: "positive",
        });
      else if (refundRate > 10)
        factors.push({
          id: "f2",
          text: "High Refund Rate",
          sentiment: "negative",
        });
    } else {
      factors.push({
        id: "f0",
        text: "Awaiting your first sale",
        sentiment: "warning",
      });
    }
