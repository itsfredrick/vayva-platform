    ...
    try {
        ...
        if (isEnabled) {
            if (!pickup?.addressLine1 || !pickup?.city || !pickup?.phone) {
                return NextResponse.json({
                    error: "Pickup address (Line 1, City) and Phone are required when delivery is enabled."
                }, { status: 400 });
            }
        }
        ...
        if (autoDispatchEnabled) {
            if (!isEnabled) {
                return NextResponse.json({ error: "Cannot enable auto-dispatch if main delivery is disabled." }, { status: 400 });
            }
            if (provider === 'KWIK' && !process.env.KWIK_API_KEY) {
                return NextResponse.json({ error: "Cannot enable auto-dispatch for Kwik without API Key." }, { status: 400 });
            }
            // Ensure pickup exists (redundant if isEnabled logic holds, but safe)
            if (!pickup?.addressLine1) {
                return NextResponse.json({ error: "Pickup address required for auto-dispatch." }, { status: 400 });
            }
        }
