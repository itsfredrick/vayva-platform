...
export async function POST(req: Request) {
    try {
        ...
        const isEnabled = await FlagService.isEnabled('templates.enabled', { merchantId: storeId });
        if (!isEnabled) {
        ...
        switch (dbPlan) {
            case 'free': currentPlan = 'free'; break;
            case 'growth': currentPlan = 'growth'; break;
            case 'pro': currentPlan = 'pro'; break;
            case 'business': currentPlan = 'pro'; break;
            case 'enterprise': currentPlan = 'pro'; break;
        }
        ...
        if (activeTemplate === templateId) {
            return NextResponse.json({ success: true, message: 'Template already active' });
        }
