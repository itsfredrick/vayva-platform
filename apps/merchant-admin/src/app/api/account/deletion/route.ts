...
export async function POST(req: NextRequest) {
    const session = await getServerSession(authOptions);
    if (!session?.user?.storeId || !session.user.id) return new NextResponse('Unauthorized', { status: 401 });

    // Verify Owner Role
    const userRole = (session.user as any).role;
    if (userRole !== 'OWNER') {
        return new NextResponse('Forbidden - Only Owner can request deletion', { status: 403 });
    }
    ...
    try {
        ...
        return NextResponse.json({ success: true, scheduledFor: result.scheduledFor });
    } catch (e: any) {
