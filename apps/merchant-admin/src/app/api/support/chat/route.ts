...
export async function POST(req: Request) {
    try {
        ...
        const ALLOWLIST = ['store_dummy_123', 'store_dev_pilot']; // Replace with real IDs
        const IS_ENABLED = process.env.SUPPORT_BOT_ENABLED === 'true' || ALLOWLIST.includes(storeId);
        ...
        if (!IS_ENABLED || MODE === 'disabled') {
            return NextResponse.json({
                ...
                message: "Our automated support is offline. Please email support@vayva.com."
            }, { status: 503 });
        }

        // Rate Limiting (30 requests / 10 mins)
        const now = Date.now();
        const limit = rateLimitMap.get(storeId) || { count: 0, resetAt: now + 600000 };
