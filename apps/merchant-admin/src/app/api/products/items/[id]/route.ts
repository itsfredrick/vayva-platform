...
export const GET = withRBAC(PERMISSIONS.COMMERCE_VIEW, async (session: any, request: Request, { params }: { params: { id: string } }) => {
    try {
        ...
        const formatted = {
            ...
            inventory: product.ProductVariant.reduce((acc: number, v: any) => acc + (v.InventoryItem[0]?.available || 0), 0),
...
export const PUT = withRBAC(PERMISSIONS.COMMERCE_MANAGE, async (session: any, request: Request, { params }: { params: { id: string } }) => {
    try {
        const storeId = session.user.storeId;
        const { id } = params;
...
export const DELETE = withRBAC(PERMISSIONS.COMMERCE_MANAGE, async (session: any, request: Request, { params }: { params: { id: string } }) => {
    try {
        const storeId = session.user.storeId;
        const { id } = params;
