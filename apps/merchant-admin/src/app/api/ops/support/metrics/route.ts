...
export async function GET(req: Request) {
    try {
        ...
        // 2. Fetch Aggregates
        // Tickets Created
        const ticketsCreated = await (prisma as any).supportTicket.count({ where: dateFilter });
        ...
        const triggerBreakdown = Object.entries(triggerCounts).map(([k, v]) => ({ trigger: k, count: v }));

        // Feedback Stats
        const feedback = await (prisma as any).supportBotFeedback.findMany({ where: dateFilter });
        ...
        const botConversations = await (prisma as any).supportTelemetryEvent.groupBy({
            by: ['conversationId'],
