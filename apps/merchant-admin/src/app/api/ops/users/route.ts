...
export async function GET(req: NextRequest) {
    ...
    if (!session || !['OPS_OWNER', 'OPS_ADMIN'].includes(session.user.role)) {
        ...
        await OpsAuthService.logEvent(session?.user?.id || null, 'OPS_UNAUTHORIZED_ACCESS', {
            ip, path: req.nextUrl.pathname, method: 'GET', reason: 'Role mismatch'
        });
...
export async function POST(req: NextRequest) {
    ...
    if (!session || session.user.role !== 'OPS_OWNER') {
        const ip = req.headers.get('x-forwarded-for')?.split(',')[0] || 'unknown';
        await OpsAuthService.logEvent(session?.user?.id || null, 'OPS_UNAUTHORIZED_ACCESS', {
            ip, path: req.nextUrl.pathname, method: 'POST', reason: 'Not Owner'
        });
        return NextResponse.json({ error: 'Unauthorized. Only Owner can create users.' }, { status: 403 });
    }
    ...
    try {
        ...
        const { user, tempPassword } = await OpsAuthService.createUser(session.user.role, data);

        await OpsAuthService.logEvent(session.user.id, 'OPS_USER_CREATED', { targetUser: user.email, role: user.role });
