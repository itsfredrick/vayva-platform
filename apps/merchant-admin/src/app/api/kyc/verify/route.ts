...
export async function POST(req: Request) {
    try {
        ...
        const isEnabled = await FlagService.isEnabled('kyc.enabled', { merchantId: storeId });
        if (!isEnabled) {
            return NextResponse.json({ error: 'Identity verification is temporarily paused' }, { status: 503 });
        }

        await checkRateLimit(userId, 'kyc_verify', 2, 3600, storeId);

        const { method, idNumber, firstName, lastName, dob, consent, selfie } = body;
