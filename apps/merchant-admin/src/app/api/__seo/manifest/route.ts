...
export async function GET() {
  if (
    ...
    process.env.SEO_DEBUG_ENABLED !== "true"
  ) {
  ...
  const manifest = await Promise.all(
    allRoutes.map(async (path) => {
      ...
      if (pt === "template_detail" && !isFamily) {
        ...
        if (t) {
          ...
          signals.bodyTextChars =
            (t.description?.length || 0) + (t.features?.length || 0) * 150;
      ...
      if (pt === "template_detail" && !isFamily) {
        const slug = path.split("/").pop() || "";
        item.primaryKeyword =
          TEMPLATE_KEYWORD_MAPS[slug]?.primary || "unknown-template-keyword";
      }
