...
export async function POST(request: NextRequest) {
    try {
        ...
        const templateSlug = properties?.templateSlug || properties?.template || properties?.templateId;
        ...
        if (eventName === 'ONBOARDING_ABANDONED') {
            console.warn(`[ALERT] Onboarding Abandoned: Template=${templateSlug} Step=${step}`);
        }

        // Alert on critical step failure
        if (eventName === 'ONBOARDING_STEP_ERROR') {
            console.error(`[ALERT] Onboarding Step Error: Template=${templateSlug} Step=${step} Error=${properties?.error}`);
        }
