...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (['viewer'].includes(role)) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
        ...
        if (shipment.provider !== 'CUSTOM') {
            return NextResponse.json({ error: "Manual status updates are only allowed for Custom Courier." }, { status: 400 });
        }
