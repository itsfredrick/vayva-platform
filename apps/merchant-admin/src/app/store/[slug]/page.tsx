    ...
    fetch("/api/products/items?status=ACTIVE&limit=4")
      ...
      .then((data) => {
        ...
        const transformedProducts = data.map((item: any) => ({
          ...
          image:
            item.images?.[0] || "https://via.placeholder.com/300?text=Product",
          slug: item.id,
      ...
      <section className="relative h-[500px] flex items-center overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10 w-full grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6 text-center md:text-left">
            <span className="inline-block px-3 py-1 rounded-full bg-white/5 border border-white/10 text-xs font-bold uppercase tracking-wider text-primary">
              New Collection 2026
            </span>
            ...
            <div className="flex gap-4 justify-center md:justify-start">
              <Link href={`/store/${slug}/collections/all`}>
                <Button
                  size="lg"
                  className="bg-primary text-black hover:bg-primary/90 rounded-full font-bold px-8"
                >
                  Shop Now
                </Button>
              </Link>
              <Link href={`/store/${slug}/collections/new`}>
                <Button
                  size="lg"
                  variant="outline"
                  className="rounded-full px-8 text-white hover:bg-white/5 border-white/20"
                >
                  View Lookbook
                </Button>
              </Link>
