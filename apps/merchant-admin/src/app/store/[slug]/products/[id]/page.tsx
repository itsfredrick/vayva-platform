...
export default function ProductDetailPage({
  ...
  const [selectedSize, setSelectedSize] = useState(DEMO_PRODUCT.sizes[1]);
    ...
    <StoreShell slug={slug}>
      <div className="max-w-7xl mx-auto px-4 py-8 md:py-16">
        ...
        <div className="text-xs text-text-secondary uppercase font-bold tracking-wider mb-8 flex items-center gap-2">
          <Link href={`/store/${slug}`} className="hover:text-white">
            Home
          </Link>
          ...
          <Icon name="ChevronRight" size={14} />
          <span className="text-white">{DEMO_PRODUCT.name}</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div>
            ...
            <div className="space-y-8 mb-10">
              {/* Colors */}
              <div>
                <label className="text-xs text-text-secondary uppercase font-bold tracking-wider mb-3 block">
              ...
              <Button
                variant="outline"
                className="w-full h-12 rounded-full border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 hover:text-emerald-300 gap-2"
              >
