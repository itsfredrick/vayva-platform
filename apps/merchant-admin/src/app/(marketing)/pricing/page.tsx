            ...
            <section className="pt-12 pb-20 px-4">
                <div className="max-w-4xl mx-auto text-center">
                    ...
                    <p className="text-xl text-[#64748B] max-w-2xl mx-auto mb-8">
                        Choose a plan that matches your business volume.
                        Every plan includes our core WhatsApp capture engine.
                    </p>
                    ...
                    <div className="inline-flex items-center gap-3 px-6 py-3 bg-red-50 border border-red-100 rounded-2xl animate-in fade-in slide-in-from-top-4 duration-700">
                        ...
                        <p className="text-sm font-bold text-red-900">
                            Honest Disclosure: A {FEES.WITHDRAWAL_PERCENTAGE}% transaction fee is charged on every withdrawal.
                        </p>
                    </div>
                </div>
            </section>
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="grid lg:grid-cols-3 gap-8">
                        {PLANS.map((plan) => (
                            <div
                                key={plan.key}
                                className={`relative flex flex-col p-8 rounded-[32px] border ${plan.featured
                            ...
                            >
                                ...
                                <div className="mb-8">
                                    ...
                                    <p className="text-sm text-gray-400 font-medium">{plan.tagline}</p>
                                </div>

                                <div className="mb-8">
                                    <div className="flex items-baseline gap-1">
                                        <span className="text-5xl font-black text-[#0F172A]">{formatNGN(plan.monthlyAmount)}</span>
                                        {plan.monthlyAmount > 0 && <span className="text-gray-400 font-bold">/mo</span>}
                                    </div>
                                    {plan.trialDays && (
                                ...
                                <Link href={plan.monthlyAmount === 0 ? '/signup' : `/signup?plan=${plan.key}`} className="block">
                                    {plan.featured ? (
                                        <PremiumButton className="w-full py-6 rounded-2xl">
                                            {plan.ctaLabel}
                                        </PremiumButton>
                                    ) : (
                                        <Button className={`w-full py-6 text-lg font-bold rounded-2xl transition-all hover:scale-[1.02] bg-gray-900 hover:bg-black text-white`}>
                                            {plan.ctaLabel}
                                        </Button>
                                    )}
                                </Link>
            ...
            <section className="py-24 px-4 bg-gray-50/50">
                <div className="max-w-6xl mx-auto">
                    <div className="text-center mb-16">
                        <h2 className="text-4xl font-black text-[#0F172A] mb-4">Compare every detail</h2>
                        <p className="text-gray-500 font-medium">No hidden limits. See exactly what you get on each plan.</p>
                    </div>
                    ...
                    <div className="mt-12 p-6 bg-white border border-gray-100 rounded-2xl shadow-sm text-center">
                        <p className="text-sm font-bold text-gray-400">
                            ...
                            Note: All plans are subject to a {FEES.WITHDRAWAL_PERCENTAGE}% transaction fee on every withdrawal from your Vayva Wallet.
                        </p>
                    </div>
                </div>
            </section>
