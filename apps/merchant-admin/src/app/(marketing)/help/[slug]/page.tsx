            ...
            <div className="max-w-3xl mx-auto">
                        ...
                        */}
                        {article.content.split('\n').map((line, i) => {
                            if (line.trim().startsWith('###')) {
                                return <h3 key={i} className="text-2xl font-bold text-gray-900 mt-8 mb-4">{line.replace('###', '').trim()}</h3>;
                            }
                            if (line.trim().startsWith('1.') || line.trim().startsWith('-')) {
                                return <li key={i} className="ml-4 mb-2">{line.trim().replace(/^[0-9\.-]+/, '').trim()}</li>;
                            }
                ...
                <div className="mt-16 pt-12 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-8">
                    ...
                    <div className="flex items-center gap-4">
                        <span className="text-sm font-bold text-gray-900">Was this helpful?</span>
                        <div className="flex gap-2">
                            <button className="px-4 py-2 rounded-lg border border-gray-200 hover:bg-green-50 hover:border-green-200 hover:text-green-600 transition-all text-sm font-bold">Yes</button>
                            <button className="px-4 py-2 rounded-lg border border-gray-200 hover:bg-red-50 hover:border-red-200 hover:text-red-600 transition-all text-sm font-bold">No</button>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-24 p-8 bg-green-50 rounded-3xl border border-green-100 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div>
                        ...
                        <p className="text-sm text-green-700">Get in touch with an expert who can walk you through it.</p>
                    </div>
                    <a
