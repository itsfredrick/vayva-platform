...
export default function TemplateDetailPage({ params }: { params: { slug: string } }) {
    ...
    const {
        name, description, previewImageDesktop,
        requiredPlan: tier, features, category
    } = template;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="p-4 rounded-xl border border-gray-100">
                                        <div className="text-gray-400 text-xs uppercase font-bold mb-1">Pages</div>
                                        <div className="font-medium text-gray-900">{creates.pages.join(', ')}</div>
                                    </div>
                                    <div className="p-4 rounded-xl border border-gray-100">
                                        <div className="text-gray-400 text-xs uppercase font-bold mb-1">Sections</div>
                                        <div className="font-medium text-gray-900">{creates.sections.join(', ')}</div>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    <div className="lg:col-span-7 space-y-8">
                        {images.map((img: string, idx: number) => (
                            <div key={idx} className="rounded-2xl overflow-hidden border border-gray-200 shadow-2xl shadow-gray-200/50 bg-gray-100">
                                ...
                                <img
                                    ...
                                    onError={(e) => {
                                        (e.target as HTMLImageElement).src = 'https://placehold.co/1200x800/f3f4f6/a1a1aa?text=High+Quality+Preview';
                                    }}
                                />
                            </div>
                        ))}
