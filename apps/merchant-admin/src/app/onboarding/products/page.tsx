...
export default function ProductsPage() {
  ...
  const addRow = () => {
    ...
    setProducts([
      ...products,
      { id: Date.now().toString(), name: "", price: "", stock: "10" },
    ]);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Get your store ready for launch. You can import more later.
          </p>
        </div>
        <div className="bg-gray-100 rounded-lg px-3 py-1 text-xs font-medium text-gray-600">
          {products.length} / {maxProducts === 999 ? "Unlimited" : maxProducts}{" "}
          used
        </div>
      </div>
            ...
            >
              <div className="col-span-6">
                <Input
                  value={product.name}
                  onChange={(e) =>
                    updateRow(product.id, "name", e.target.value)
                  }
              ...
              <div className="col-span-2">
                <Input
                  value={product.stock}
                  onChange={(e) =>
                    updateRow(product.id, "stock", e.target.value)
                  }
                  type="number"
                  className="!h-10 text-sm"
                />
      ...
      <div className="flex justify-between items-center pt-4">
        <Button variant="ghost" onClick={() => goToStep("templates")}>
          Back
        </Button>
