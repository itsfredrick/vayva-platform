...
export default function PaymentsPage() {
    ...
    const methodOptions: { id: PaymentMethod; label: string; icon: string; desc: string }[] = [
        { id: 'transfer', label: 'Bank Transfer', icon: 'Landmark', desc: 'Direct deposit to your account' },
        { id: 'cash', label: 'Cash', icon: 'Coins', desc: 'Physical cash on pickup/delivery' },
        { id: 'pos', label: 'POS Terminal', icon: 'CreditCard', desc: 'Card machine payment' },
        { id: 'mixed', label: 'Mixed / Other', icon: 'Wallet', desc: 'Multiple methods allowed' },
    ];
    ...
    return (
        <div className="flex flex-col lg:flex-row h-full gap-8 max-w-6xl mx-auto items-start">
            <div className="flex-1 w-full max-w-lg lg:pt-10">
                <div className="mb-8">
                    <h1 className="text-2xl font-bold text-gray-900 mb-2">How do you accept payment?</h1>
                    <p className="text-gray-500">Select all methods that apply. We'll show these to your customers.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    {methodOptions.map((opt) => (
                        ...
                        >
                            ...
                            <span className={cn("text-[10px]", methods.includes(opt.id) ? "text-gray-400" : "text-gray-400")}>
                                {opt.desc}
                            </span>
                        </button>
                    ))}
                ...
                {methods.length > 0 && (
                    <div className="space-y-6 bg-gray-50 p-6 rounded-2xl border border-gray-100 mb-8 animate-in slide-in-from-top-4 fade-in">
                        ...
                        <div className="space-y-3">
                            <h4 className="font-bold text-gray-900 text-sm">Payment Proof Rules</h4>
                            <div className="flex flex-col gap-2">
                                {([
                                    { id: 'optional', label: 'Optional', desc: 'Customers can skip upload' },
                                    { id: 'required_transfer', label: 'Required for Transfers', desc: 'Only for bank transfers' },
                                    { id: 'required_all', label: 'Always Required', desc: 'Strict verification' },
                                ] as const).map(rule => (
                                    <button
                                        key={rule.id}
                                    ...
                                    >
                                        <div className={cn(
                                            ...
                                            proofRule === rule.id ? "border-black" : "border-gray-300"
                                        )}>
                                            {proofRule === rule.id && <div className="w-2 h-2 bg-black rounded-full" />}
                                        </div>
                                        <div className="text-left">
                                            <span className="block font-medium">{rule.label}</span>
                                            <span className="text-[10px] text-gray-400">{rule.desc}</span>
                                        </div>
                                    </button>
                                ))}
                            </div>
                        ...
                        <div className={cn("pt-6 border-t border-gray-200", isFree && "opacity-80")}>
                            <div className="flex items-start justify-between">
                                <div className="space-y-1">
                                    <div className="flex items-center gap-2">
                                        <h4 className="font-bold text-gray-900 text-sm">Auto-Reconciliation</h4>
                                        {isFree && <span className="text-[10px] bg-gray-200 px-1.5 py-0.5 rounded text-gray-600 font-bold">GROWTH</span>}
                                    </div>
            ...
            <div className="hidden lg:block flex-1 w-full sticky top-24">
                <div className="bg-white rounded-3xl border border-gray-100 shadow-xl p-6 min-h-[500px] flex flex-col">
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-8 text-center">Customer Checkout Preview</h3>
                    ...
                    <div className="max-w-sm mx-auto w-full bg-gray-50 rounded-2xl p-4 border border-gray-100 shadow-sm relative overflow-hidden">
                        ...
                        <div className="space-y-3">
                            <h4 className="text-xs font-bold text-gray-500 uppercase">Select Payment</h4>
                                ...
                                {methods.length > 0 ? methods.map(m => (
                                    <div
                                        ...
                                        className={cn(
                                            ...
                                            activePreview === m ? "bg-white border-green-500 shadow-sm" : "bg-white border-gray-200 opacity-70"
                                        )}
                                    >
                                        <div className={cn("w-4 h-4 rounded-full border", activePreview === m ? "border-green-500 border-4" : "border-gray-300")} />
                                        <div className="flex-1 text-sm font-medium capitalize flex items-center gap-2">
                                            {m.replace('_', ' ')}
                                            {m === 'transfer' && <span className="text-[10px] bg-blue-100 text-blue-700 px-1 rounded">Fastest</span>}
                                        </div>
                                    </div>
                                )) : (
                                    <div className="text-center py-4 text-gray-400 text-xs italic">Select methods to see them appear here</div>
                                )}
                            </div>
                        </div>
