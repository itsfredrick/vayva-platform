      ...
      <div className="flex-1 w-full max-w-lg lg:pt-10">
        ...
        <div className="space-y-4 mb-8">
          {[
            {
              id: "required",
              label: "Yes, delivery is required",
              desc: "Customers must enter address",
              icon: "Truck",
            },
            {
              id: "sometimes",
              label: "Yes, but pickup available",
              desc: "Customer chooses delivery or pickup",
              icon: "Warehouse",
            },
            {
              id: "pickup_only",
              label: "No, pickup only",
              desc: "Physical location or services",
              icon: "Store",
            },
          ].map((opt) => (
            <button
        ...
        {policy !== "pickup_only" && (
          <div className="space-y-6 animate-in slide-in-from-top-4 fade-in">
                ...
                {stages.map((stage, idx) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() => {
                        const newStages = [...stages];
                        newStages[idx].visibleToCustomer =
                          !newStages[idx].visibleToCustomer;
                        setStages(newStages);
                      }}
                      className={cn(
                    ...
                    >
                      <Icon
                        name={stage.visibleToCustomer ? "Eye" : "EyeOff"}
                        size={14}
                      />
                    </button>
                  </div>
            ...
            <div
              className={cn(
                "p-4 bg-gray-50 rounded-xl border border-gray-100 flex items-center justify-between",
                !isPro && "opacity-75",
              )}
            >
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Riders must upload photo/signature.
                </p>
              </div>
              <button
          ...
          {proof && policy !== "pickup_only" && (
            <div className="mt-auto bg-gray-50 p-4 rounded-xl border border-gray-100">
              ...
              <div className="flex gap-2">
                <div className="w-16 h-16 bg-gray-200 rounded-lg flex items-center justify-center text-gray-400 text-[10px]">
                  Photo
                </div>
                <div className="w-16 h-16 bg-white border border-gray-200 rounded-lg flex items-center justify-center text-gray-400 font-script italic">
                  Sign
                </div>
              </div>
            </div>
