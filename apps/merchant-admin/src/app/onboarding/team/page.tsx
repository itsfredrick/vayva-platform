        ...
        <div className="flex flex-col lg:flex-row h-full gap-8 max-w-6xl mx-auto items-start">
            <div className="flex-1 w-full max-w-lg lg:pt-10">
                <div className="mb-6">
                    <div className="flex items-center gap-2 mb-2">
                        <span className="bg-gray-100 text-gray-500 text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wide">Step 9 of 11</span>
                    </div>
                    <h1 className="text-2xl font-bold text-gray-900 mb-2">Who else manages this business?</h1>
                    <p className="text-gray-500 text-sm">You can invite team members now or do it later.</p>
                </div>

                <div className="space-y-4 mb-8 bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                    <label className="text-sm font-bold text-gray-900">Invite by Email</label>
                    <div className="flex gap-2">
                        ...
                        <select
                            value={role}
                            onChange={(e) => { setRole(e.target.value as Role); setActiveRolePreview(e.target.value as Role); }}
                    ...
                    <Button onClick={addInvite} disabled={!email} className="w-full bg-black text-white rounded-lg">
                        Add Invitation
                    </Button>
                    ...
                    {invites.length > 0 && (
                        <div className="space-y-2 mt-4 pt-4 border-t border-gray-100">
                            {invites.map((inv) => (
                                <div key={inv.email} className="flex items-center justify-between text-sm bg-gray-50 p-3 rounded-lg border border-gray-200">
                ...
                <div className="flex flex-col gap-3">
                    ...
                    >
                        {invites.length > 0 ? `Send ${invites.length} Invites & Continue` : 'Continue as Solo'}
                    </Button>

                    {invites.length === 0 && (
                        <button onClick={handleSkip} className="text-gray-400 text-sm hover:text-black hover:underline">
                            Skip this step
                        </button>
                    )}
                </div>
            ...
            <div className="hidden lg:block flex-1 w-full sticky top-24">
                <div className="bg-white rounded-3xl border border-gray-100 shadow-xl p-8 transition-all">
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-6">Permission Preview</h3>

                    <div className="space-y-4">
                        {[
                            { id: 'admin', icon: 'Shield', label: 'Admin', desc: 'Full access to settings, withdrawals, and team management.' },
                            { id: 'staff', icon: 'User', label: 'Staff', desc: 'Can manage orders, products, and chat. No withdrawals.' },
                            { id: 'viewer', icon: 'Eye', label: 'Viewer', desc: 'Read-only access to orders and reports.' },
                        ].map((r) => (
                            <div
                                key={r.id}
                                className={cn(
                            ...
                            >
                                <div className="flex items-center gap-3 mb-2">
                                    <Icon name={r.icon as any} size={18} className={activeRolePreview === r.id ? "text-green-400" : "text-gray-400"} />
