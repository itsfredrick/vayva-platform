...
export const OrderDetailsDrawer = ({ order, onClose }: OrderDetailsDrawerProps) => {
    ...
    const handleRefund = async () => {
        ...
        try {
            await fetch(`/api/orders/${order.id}/refund`, {
                method: 'POST',
                body: JSON.stringify({ amount: order.totalAmount, reason: actionNote || 'Full refund' })
            });
            onClose();
        } catch (e) {
            console.error(e);
    ...
    const renderActionButtons = () => {
        if (showCancel) {
            return (
                <div className="flex flex-col gap-3 w-full animate-in slide-in-from-bottom-5">
                    <p className="text-sm font-bold text-red-600 mb-1">Confirm Cancellation?</p>
                    <input
                    ...
                    <div className="flex gap-2">
                        <button onClick={() => setShowCancel(false)} className="flex-1 py-3 bg-gray-100 font-bold rounded-xl hover:bg-gray-200">Back</button>
                        <button onClick={handleCancel} disabled={isLoading} className="flex-1 py-3 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700">
        ...
        if (showRefund) {
            return (
                <div className="flex flex-col gap-3 w-full animate-in slide-in-from-bottom-5">
                    <p className="text-sm font-bold text-orange-600 mb-1">Issue Full Refund?</p>
                    <input
                        className="w-full border border-gray-300 rounded-lg p-2 text-sm"
                    ...
                    />
                    <div className="flex gap-2">
                        <button onClick={() => setShowRefund(false)} className="flex-1 py-3 bg-gray-100 font-bold rounded-xl hover:bg-gray-200">Back</button>
                        <button onClick={handleRefund} disabled={isLoading} className="flex-1 py-3 bg-orange-500 text-white font-bold rounded-xl hover:bg-orange-600">
                            {isLoading ? 'Processing...' : `Refund ${formatCurrency(order.totalAmount)}`}
                        </button>
                    </div>
                </div>
            )
            ...
            <div className="flex flex-col gap-3 w-full">
                <div className="flex gap-2">
                    {order.status === UnifiedOrderStatus.NEW && (
                        <>
                            <button
                                onClick={() => handleStatusUpdate(UnifiedOrderStatus.PROCESSING)}
                ...
                {/* Secondary Actions */}
                {order.status !== UnifiedOrderStatus.CANCELLED && order.status !== UnifiedOrderStatus.REFUNDED && order.status !== UnifiedOrderStatus.NEW && (
                    <div className="flex gap-2 justify-center pt-2">
                        <button onClick={() => setShowCancel(true)} className="text-xs font-bold text-red-500 hover:text-red-700 underline">Cancel Order</button>
                        <span className="text-gray-300">|</span>
                        <button onClick={() => setShowRefund(true)} className="text-xs font-bold text-orange-500 hover:text-orange-700 underline">Issue Refund</button>
                    </div>
                )}
            </div>
        );
    ...
    return (
        <Drawer
            isOpen={!!order}
            onClose={onClose}
            title={order.type === OrderType.SERVICE ? `Booking #${order.id.split('_')[1]}` : `Order #${order.id.split('_')[1]}`}
            ...
            <div className="p-6 space-y-8 pb-32">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <h2 className="text-xl font-bold capitalize">{order.status.replace('_', ' ')}</h2>
                    </div>
                    <div className={cn(
                        "px-4 py-2 rounded-full text-sm font-bold capitalize",
                        order.status === UnifiedOrderStatus.NEW ? "bg-blue-100 text-blue-700" :
                            order.status === UnifiedOrderStatus.COMPLETED ? "bg-green-100 text-green-700" :
                                order.status === UnifiedOrderStatus.CANCELLED ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-700"
                    )}>
                                        ...
                                        {item.modifiers && (
                                            <div className="flex flex-wrap gap-1 mt-1">
                                                {item.modifiers.map((m, idx) => (
                                                    <span key={idx} className="text-[10px] bg-gray-100 text-gray-600 px-1.5 rounded">{m}</span>
                                                ))}
                                            </div>
                                        )}
                                    </div>
                                </div>
                                <span className="font-mono text-sm font-medium">{formatCurrency(item.price * item.quantity)}</span>
                            </div>
                        ))}
