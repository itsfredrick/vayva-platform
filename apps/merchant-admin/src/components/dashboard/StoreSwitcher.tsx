    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch('/api/merchant/stores', {
                method: 'POST',
                body: JSON.stringify({ name: newName, slug: newSlug, category: 'general' })
            });
            if (res.ok) {
        ...
        <div className="relative">
            ...
            {isOpen && (
                <div className="absolute top-full left-0 mt-2 w-64 bg-white border border-gray-200 rounded-xl shadow-lg p-2 z-50">
                    <div className="text-xs font-bold text-gray-400 px-2 py-1 uppercase tracking-wider mb-1">Switch Store</div>

                    {stores.map(s => (
                        <button
                            key={s.id}
                            onClick={() => handleSwitch(s)}
                            className={`w-full text-left px-3 py-2 rounded-lg text-sm flex items-center gap-2 ${s.id === activeStore.id ? 'bg-gray-50 font-bold text-black' : 'text-gray-500 hover:bg-gray-50'
                                }`}
                        >
                            <span className={`w-2 h-2 rounded-full ${s.id === activeStore.id ? 'bg-green-500' : 'bg-gray-300'}`} />
                            {s.name}
                        </button>
                    ))}
            ...
            {showCreate && (
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100]">
                    <form onSubmit={handleCreate} className="bg-white p-6 rounded-xl w-full max-w-sm">
                        ...
                        <div className="space-y-4 mb-6">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full border p-2 rounded" placeholder="My New Store"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">URL Slug</label>
                                <div className="flex items-center border rounded overflow-hidden">
                                    <span className="bg-gray-50 px-3 py-2 text-gray-500 text-sm">vayva.ng/</span>
                                    <input
                                        required
                                        value={newSlug} onChange={e => setNewSlug(e.target.value)}
                                        className="flex-1 p-2 outline-none" placeholder="new-store"
                                    />
                                </div>
                            </div>
                        </div>
                        <div className="flex justify-end gap-2">
                            <button type="button" onClick={() => setShowCreate(false)} className="px-4 py-2 text-gray-500 font-bold">Cancel</button>
                            <button type="submit" disabled={creating} className="bg-black text-white px-4 py-2 rounded-lg font-bold">
