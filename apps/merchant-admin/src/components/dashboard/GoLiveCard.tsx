...
export function GoLiveCard() {
    ...
    async function handleGoLive() {
        setProcessing(true);
        try {
            const res = await fetch('/api/merchant/store/publish/go-live', { method: 'POST' });
            const data = await res.json();
            if (res.ok) {
                // Success
                fetchStatus();
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <div className="flex justify-between items-start mb-4">
                ...
                <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${isLive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'
                    }`}>
                    {isLive ? 'Live' : 'Offline'}
                </div>
            </div>
            ...
            {isLive ? (
                <div className="space-y-4">
                    <div className="p-4 bg-green-50 rounded-lg flex items-center gap-3">
                        ...
                        <button onClick={handleUnpublish} disabled={processing} className="text-xs text-red-600 font-bold hover:underline">
                            Unpublish
                        </button>
                    </div>
                </div>
                    ...
                    {!isReady && (
                        <div className="bg-orange-50 p-4 rounded-lg border border-orange-100">
                            ...
                            <div className="space-y-1">
                                {blockers.map((b: any) => (
                                    <div key={b.code} className="text-xs text-orange-700 flex justify-between">
                                        <span>• {b.title}</span>
                                        {b.actionUrl && <a href={b.actionUrl} className="underline hover:text-orange-900">Fix</a>}
                                    </div>
                                ))}
                            </div>
                        </div>
