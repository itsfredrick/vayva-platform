...
export function DashboardSetupChecklist() {
    ...
    const shouldShow = merchant.onboardingStatus === OnboardingStatus.OPTIONAL_INCOMPLETE || merchant.onboardingStatus === OnboardingStatus.REQUIRED_COMPLETE;
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-8 animate-in slide-in-from-top-4 fade-in duration-500">
            <div className="flex justify-between items-start mb-4">
                <div>
                    ...
                    <p className="text-gray-500 text-sm">Complete these steps to get the most out of Vayva.</p>
                </div>
                <div className="flex items-center gap-4">
                    <span className="text-sm font-bold text-gray-900">{progressPercent}% Done</span>
                    <button
                ...
                {items.map(item => (
                    ...
                    >
                        <div className={cn(
                            ...
                            item.isDone ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-500 group-hover:bg-black group-hover:text-white transition-colors"
                        )}>
                            {item.isDone ? <Icon name="Check" size={20} /> : <Icon name="ArrowRight" size={20} />}
                        </div>
                        <div className="flex-1">
                            <h4 className={cn("font-bold text-sm", item.isDone ? "text-gray-500 line-through" : "text-gray-900")}>
                                {item.label}
                            </h4>
