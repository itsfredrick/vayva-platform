...
const MotionCard = ({ children, className, onClick }: { children: React.ReactNode, className?: string, onClick?: () => void }) => (
    <motion.div
        className={cn("bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden cursor-pointer", className)}
        whileHover={{ y: -3, scale: 1.01, boxShadow: "0 10px 30px -10px rgba(0,0,0,0.05)" }}
        whileTap={{ scale: 0.98 }}
        onClick={onClick}
    >
        {children}
...
// B2) KPI Card V2
export const KPICardV2 = ({ title, value, delta, isPositive, isPrimary }: any) => (
    <motion.div
        className={cn(
    ...
    >
        <div className="flex items-center justify-between">
            ...
            {isPrimary && <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />}
        </div>
        <div className="flex items-baseline gap-3">
            <span className="text-3xl font-bold text-[#0B0B0B] tracking-tight">{value}</span>
            {delta && (
                <span className={cn(
                    "text-xs font-bold px-1.5 py-0.5 rounded flex items-center gap-0.5",
                    isPositive ? "text-green-600 bg-green-50" : "text-gray-400 bg-gray-50"
                )}>
                    {isPositive ? '↑' : ''} {delta}
                </span>
            )}
...
export const ActionTileV2 = ({ icon, label, href, available = true }: any) => {
    const Content = (
        <MotionCard className={cn(
            ...
            !available && "opacity-60 grayscale cursor-not-allowed hover:y-0 hover:scale-100"
        )}>
            <div className={cn(
                "w-10 h-10 rounded-full flex items-center justify-center transition-colors",
                available ? "bg-gray-50 text-[#0B0B0B] group-hover:bg-[#0B0B0B] group-hover:text-white" : "bg-gray-100 text-gray-400"
            )}>
                {/* @ts-ignore */}
                <Icon name={icon} size={20} />
            </div>
            <span className="text-sm font-semibold text-[#525252] group-hover:text-[#0B0B0B] transition-colors">{label}</span>
            {!available && (
                <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Soon</span>
            )}
        </MotionCard>
    );
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 flex flex-col h-full gap-6">
            <div className="flex items-center gap-4">
                <div
                    ...
                    style={{ backgroundColor: store.brandColor ? `${store.brandColor}10` : undefined }}
                >
                    {store.logoUrl ? (
                ...
                <div>
                    <h3 className="text-lg font-bold text-[#0B0B0B]">{store.storeName}</h3>
                    <a href={storeUrl} target="_blank" className="text-sm text-gray-500 hover:text-[#0B0B0B] hover:underline flex items-center gap-1">
            ...
            <div className="flex gap-2">
                <div className={cn(
                    ...
                    isPublished ? "bg-green-50 text-green-700 border-green-200" : "bg-yellow-50 text-yellow-700 border-yellow-200"
                )}>
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col h-full">
            <div className="p-5 border-b border-gray-50 flex items-center justify-between">
                ...
                <button className="text-xs font-semibold text-gray-500 hover:text-[#0B0B0B]">View All</button>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-xs text-gray-400 max-w-[200px]">Orders and payouts will appear here once you start selling.</p>
            </div>
        </div>
    )
}
