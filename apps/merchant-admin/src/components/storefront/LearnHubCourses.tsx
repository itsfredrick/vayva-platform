...
export function LearnHubCourses({ storeName: initialStoreName, storeSlug }: { storeName: string, storeSlug?: string }) {
        ...
        <div className="bg-[#f8f9fa] min-h-screen font-sans text-[#333]">
            ...
            <nav className="bg-blue-600 text-white sticky top-0 z-50 shadow-lg">
                <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-6 text-sm font-medium">
                        <a href="#" className="opacity-90 hover:opacity-100">Degrees</a>
                        <a href="#" className="opacity-90 hover:opacity-100">Certificates</a>
                        <a href="#" className="opacity-90 hover:opacity-100">For Business</a>
                    </div>

                    <div className="flex items-center gap-4">
                        <button className="text-sm font-bold opacity-90 hover:opacity-100">Log In</button>
                        <button
            ...
            {isCartOpen && (
                <div className="fixed inset-0 z-50 flex justify-end">
                    <div className="absolute inset-0 bg-blue-900/30 backdrop-blur-sm" onClick={() => setIsCartOpen(false)} />
                    <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right">
                        <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                            <h2 className="font-bold text-xl text-blue-900">Your Learning Plan</h2>
            ...
            <div className="bg-white border-b border-gray-200">
                <div className="max-w-7xl mx-auto px-6 py-20 flex items-center">
                    <div className="max-w-2xl">
                        <h1 className="text-5xl md:text-6xl font-bold mb-6 text-[#1f1f1f] tracking-tight">Learn without limits</h1>
                        <p className="text-xl text-gray-600 mb-8 max-w-lg">Build skills with courses, certificates, and degrees online from world-class universities and companies.</p>
                        <div className="flex gap-4">
                            <button className="bg-blue-600 text-white px-8 py-4 font-bold rounded text-lg shadow-blue-200 shadow-xl hover:bg-blue-700 transition-colors">Start Learning Free</button>
                            <button className="border border-blue-600 text-blue-600 px-8 py-4 font-bold rounded text-lg hover:bg-blue-50 transition-colors">Career Guide</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-gray-100 border-b border-gray-200 py-12">
                <div className="max-w-7xl mx-auto px-6 flex justify-around text-center">
                    <div>
                        ...
                        <div className="text-xs font-bold text-gray-500 uppercase tracking-widest">Report Career Benefits</div>
                    </div>
                    <div>
                        <div className="text-3xl font-bold text-blue-600 mb-1">100+</div>
                        <div className="text-xs font-bold text-gray-500 uppercase tracking-widest">Global Partners</div>
                    </div>
                    <div>
                        <div className="text-3xl font-bold text-blue-600 mb-1">24/7</div>
                        <div className="text-xs font-bold text-gray-500 uppercase tracking-widest">Online Support</div>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-6 py-16">
                <h2 className="text-2xl font-bold mb-8 text-[#1f1f1f]">Most Popular Certificates</h2>
                ...
                {isLoading && products.length === 0 ? (
                    ...
                    <div className="grid md:grid-cols-4 gap-6">
                        {products.map(course => (
                            <div key={course.id} className="bg-white shadow-sm border border-gray-200 rounded-xl overflow-hidden hover:shadow-xl transition-all h-full flex flex-col group cursor-pointer" onClick={() => addToCart(course)}>
                                <div className="h-40 bg-gray-200 relative">
                                    <img
                                        src={course.image || `https://via.placeholder.com/300x200?text=${encodeURIComponent(course.name)}`}
                                ...
                                <div className="p-6 flex-1 flex flex-col">
                                    ...
                                    <h3 className="font-bold text-lg text-gray-900 mb-2 line-clamp-2">{course.name}</h3>

                                    <div className="flex items-center gap-1 text-sm font-bold text-yellow-500 mb-4">
                                        4.7 <Star className="w-4 h-4 fill-current" />
                                        <span className="text-gray-400 font-normal ml-1 text-xs">(45k reviews)</span>
                                    </div>

                                    <p className="text-xs text-gray-500 mb-6 line-clamp-2">{course.description}</p>

                                    <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                                        <div className="font-bold text-lg text-blue-700">₦{course.price.toLocaleString()}</div>
