...
export function AAFashionHome({ storeName: initialStoreName, storeSlug }: { storeName: string, storeSlug?: string }) {
    ...
    return (
        <div className="bg-black text-white min-h-screen font-sans selection:bg-white selection:text-black">
            ...
            <nav className="fixed w-full z-50 mix-blend-difference px-8 py-6 flex justify-between items-center">
                <div className="text-2xl font-bold tracking-tighter uppercase">{displayName}</div>
                <div className="flex gap-8 text-sm font-medium tracking-wide">
                    <button className="hover:underline underline-offset-4">Collection</button>
                    <button className="hover:underline underline-offset-4">Editorial</button>
                    <button
                ...
                <div className="fixed inset-0 z-[60] flex justify-end">
                    ...
                    <div className="relative w-full max-w-md bg-[#111] border-l border-gray-800 h-full p-6 flex flex-col animate-in slide-in-from-right">
                        <div className="flex justify-between items-center mb-8">
                            <h2 className="text-xl font-bold uppercase tracking-widest">Your Bag</h2>
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="flex items-center gap-4">
                                                <div className="flex items-center border border-gray-800 rounded">
                                                    <button onClick={() => updateQuantity(item.id, -1)} className="p-1 hover:bg-white hover:text-black transition-colors"><Minus className="w-3 h-3" /></button>
                                                    <span className="px-2 text-xs">{item.quantity}</span>
                                                    <button onClick={() => updateQuantity(item.id, 1)} className="p-1 hover:bg-white hover:text-black transition-colors"><Plus className="w-3 h-3" /></button>
                                                </div>
                                                <button onClick={() => removeFromCart(item.id)} className="text-xs text-gray-500 hover:text-white underline">Remove</button>
                                            </div>
                                        </div>
                                    </div>
                                ))
            ...
            <header className="relative h-screen flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-20 text-center max-w-4xl mx-auto px-6">
                    <p className="text-sm md:text-base mb-4 font-bold tracking-[0.3em] uppercase opacity-80">Season 04 / 24</p>
            ...
            <section className="py-32 px-4 md:px-8 max-w-[1600px] mx-auto">
                <div className="flex justify-between items-end mb-16">
                    <h2 className="text-4xl md:text-6xl font-bold tracking-tighter">New Arrivals</h2>
                    <div className="text-right hidden md:block">
                        <p className="text-gray-400 text-sm tracking-widest uppercase">Limited Edition Drop</p>
                    </div>
                </div>
                        ...
                        {products.map((product) => (
                            <div key={product.id} className="group cursor-pointer">
                                <div className="aspect-[3/4] bg-gray-900 mb-6 overflow-hidden relative">
                                    <img
                                        src={product.image || `https://via.placeholder.com/600x800/111/444?text=${encodeURIComponent(product.name)}`}
                                        alt={product.name}
                                ...
                                <div className="flex justify-between items-start">
                                    <div>
                                        ...
                                        <p className="text-gray-500 text-xs">{product.category || 'Apparel'}</p>
                                    </div>
                                    <span className="font-mono text-sm opacity-60">₦{product.price.toLocaleString()}</span>
                                </div>
                            </div>
                        ))}
                    </div>
