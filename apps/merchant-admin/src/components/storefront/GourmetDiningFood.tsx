...
export function GourmetDiningFood({ storeName: initialStoreName, storeSlug }: { storeName: string, storeSlug?: string }) {
    const { store } = useStorefrontStore(storeSlug);
    const { products, isLoading } = useStorefrontProducts(storeSlug, { limit: 50 });
    ...
    return (
        <div className="bg-[#1c1c1c] min-h-screen text-[#e5e5e5] font-serif">
            ...
            <nav className="absolute top-0 w-full z-50 flex justify-between items-center px-8 md:px-16 py-8">
                <div className="text-2xl font-bold tracking-widest uppercase">{displayName}</div>
                ...
                <div className="flex gap-6">
                    <button className="text-xs font-sans tracking-[0.2em] uppercase border border-gold px-6 py-2 hover:bg-[#d4af37] hover:text-black transition-colors"
                    ...
                    >
                        ...
                        {cart.length > 0 && <span className="absolute top-0 right-0 w-2 h-2 bg-red-500 rounded-full"></span>}
                    </button>
                </div>
            </nav>
                ...
                <div className="fixed inset-0 z-50 flex justify-end font-sans text-gray-900">
                    ...
                    <div className="relative w-full max-w-md bg-white h-full p-8 flex flex-col animate-in slide-in-from-right duration-500">
                        ...
                        <div className="flex-1 overflow-auto space-y-6 pr-2">
                            {cart.length === 0 ? (
                                <div className="text-center py-20 text-gray-400">Your order is empty.</div>
                            ) : (
                                cart.map(item => (
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="flex items-center gap-4 mt-2">
                                                <div className="flex items-center border border-gray-200 rounded-full px-2">
                                                    <button onClick={() => updateQuantity(item.id, -1)} disabled={item.quantity <= 1} className="p-1 hover:text-orange-500 disabled:opacity-30"><Minus className="w-3 h-3" /></button>
                                                    <span className="px-3 text-sm">{item.quantity}</span>
                                                    <button onClick={() => updateQuantity(item.id, 1)} className="p-1 hover:text-orange-500"><Plus className="w-3 h-3" /></button>
                                                </div>
            ...
            <header className="relative h-screen flex items-center justify-center">
                ...
                <div className="relative z-10 text-center px-4">
                    <div className="text-[#d4af37] text-sm md:text-base font-sans tracking-[0.3em] uppercase mb-6 animate-fade-in">Fine Dining Experience</div>
                    <h1 className="text-5xl md:text-8xl font-bold mb-8 tracking-tight">{displayName}</h1>
                    <p className="max-w-xl mx-auto text-gray-400 mb-12 font-sans font-light leading-relaxed">
                        A culinary journey through contemporary flavors and traditional techniques.
                    </p>
                    <button
            ...
            <div className="bg-[#161616] border-y border-white/5 py-12">
                <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-3 gap-8 text-center font-sans">
                    <div className="flex flex-col items-center">
                        <div className="text-[#d4af37] mb-4"><Clock className="w-6 h-6" /></div>
                        <h3 className="text-sm font-bold uppercase tracking-widest mb-2">Opening Hours</h3>
                        <p className="text-gray-500 text-sm">Mon-Sun: 17:00 - 23:00</p>
                    </div>
                    <div className="flex flex-col items-center">
                        <div className="text-[#d4af37] mb-4"><MapPin className="w-6 h-6" /></div>
                    ...
                    <div className="flex flex-col items-center">
                        <div className="text-[#d4af37] mb-4"><Star className="w-6 h-6" /></div>
                        <h3 className="text-sm font-bold uppercase tracking-widest mb-2">Michelin Guide</h3>
                ...
                {isLoading && products.length === 0 ? (
                    ...
                    <div className="space-y-20">
                        {Object.entries(menuSections).map(([category, items]) => (
                            <div key={category}>
                                <h3 className="text-3xl font-serif text-[#d4af37] mb-8 border-b border-white/10 pb-4 inline-block pr-12">{category}</h3>
                                <div className="grid md:grid-cols-2 gap-x-16 gap-y-12">
                                    {items.map(dish => (
                                        <div key={dish.id} className="group cursor-pointer">
                                            <div className="flex justify-between items-baseline mb-2">
                                                <h4 className="text-xl font-bold group-hover:text-[#d4af37] transition-colors">{dish.name}</h4>
                                            ...
                                            <p className="text-gray-500 text-sm italic mb-3">{dish.description}</p>
                                            <button
            ...
            <section id="reservations" className="py-24 bg-[#161616] mt-24">
                <div className="max-w-4xl mx-auto px-6 text-center">
                    ...
                    <p className="text-gray-400 mb-10 max-w-xl mx-auto">
                        Book your table for an unforgettable evening. For private events, please contact us directly.
                    </p>
                    <div className="flex flex-col md:flex-row gap-4 justify-center">
                        <input type="date" className="bg-[#1c1c1c] border border-white/10 text-white px-6 py-4 rounded-none focus:border-[#d4af37] outline-none" />
