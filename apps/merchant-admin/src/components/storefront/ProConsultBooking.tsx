...
import { ShoppingBag, X, Calendar, Linkedin, ArrowRight, Plus, Minus, Video } from 'lucide-react';

export function ProConsultBooking({ storeName: initialStoreName, storeSlug }: { storeName: string, storeSlug?: string }) {
        ...
        <div className="bg-white min-h-screen font-sans text-slate-800">
            ...
            <nav className="border-b border-slate-100 flex justify-between items-center px-8 py-5 sticky top-0 bg-white/90 backdrop-blur z-50">
                ...
                <div className="hidden md:flex gap-8 text-sm font-semibold text-slate-500">
                    <a href="#" className="hover:text-blue-600">Experts</a>
                    <a href="#" className="hover:text-blue-600">Services</a>
                    <a href="#" className="hover:text-blue-600">Enterprise</a>
                </div>
                <div className="flex gap-4">
                    <button className="text-sm font-bold text-slate-600 hover:text-blue-600">Login</button>
                    <button
                ...
                <div className="fixed inset-0 z-50 flex justify-end">
                    ...
                    <div className="relative w-full max-w-md bg-white h-full p-6 flex flex-col shadow-2xl animate-in slide-in-from-right">
                        <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
                            <h2 className="text-xl font-bold text-slate-900">Session Summary</h2>
                                ...
                                cart.map(item => (
                                    <div key={item.id} className="border border-slate-200 rounded-lg p-4 hover:border-blue-200 transition-colors">
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <span className="font-bold text-blue-600 text-sm">₦{(item.price * item.quantity).toLocaleString()}</span>
                                        </div>
                                        <div className="flex items-center justify-between mt-3">
                                            ...
                                            <div className="flex items-center gap-3">
                                                <div className="flex items-center border border-slate-200 rounded px-1">
                                                    <button onClick={() => updateQuantity(item.id, -1)} className="p-1 hover:text-blue-600"><Minus className="w-3 h-3" /></button>
                                                    <span className="text-xs px-2">{item.quantity}</span>
                                                    <button onClick={() => updateQuantity(item.id, 1)} className="p-1 hover:text-blue-600"><Plus className="w-3 h-3" /></button>
                                                </div>
                ...
                <div className="max-w-6xl mx-auto px-8 relative z-10 grid md:grid-cols-2 gap-12 items-center">
                    <div>
                        ...
                        <h1 className="text-5xl font-bold mb-6 leading-tight">
                            Expert advice,<br />on demand.
                        </h1>
                        <p className="text-lg text-slate-300 mb-8 max-w-md">
                            Connect with top-tier consultants across tech, marketing, legal, and finance via high-quality video calls.
                        </p>
                        <div className="flex gap-4">
            ...
            <section className="py-24 max-w-7xl mx-auto px-8">
                <div className="flex justify-between items-end mb-12">
                    <div>
                        <h2 className="text-3xl font-bold text-slate-900 mb-2">Available Consultants</h2>
                ...
                {isLoading && products.length === 0 ? (
                    ...
                    <div className="grid md:grid-cols-3 gap-8">
                        {products.map(expert => (
                            <div key={expert.id} className="border border-slate-200 rounded-xl hover:shadow-xl hover:border-blue-100 transition-all bg-white group cursor-pointer" onClick={() => addToCart(expert)}>
                                <div className="p-8">
                                    <div className="flex justify-between items-start mb-6">
                                        <div className="w-16 h-16 bg-slate-100 rounded-full overflow-hidden border-2 border-white shadow-sm">
                                            <img src={expert.image || `https://via.placeholder.com/100x100?text=${expert.name.charAt(0)}`} className="w-full h-full object-cover" />
                                        </div>
                                    ...
                                    <p className="text-sm text-slate-500 mb-4 line-clamp-2 min-h-[40px]">{expert.description || "Senior Consultant specialized in business strategy and growth."}</p>

                                    <div className="flex gap-2 mb-6">
                                        <span className="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded font-medium">Strategy</span>
                                        <span className="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded font-medium">Growth</span>
                                    </div>

                                    <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                                        <div>
                                            <span className="block text-xs text-slate-400 uppercase tracking-wider font-bold">Rate</span>
                                            <div className="text-slate-900 font-bold">₦{expert.price.toLocaleString()} <span className="text-xs font-normal text-slate-400">/ session</span></div>
                                        </div>
