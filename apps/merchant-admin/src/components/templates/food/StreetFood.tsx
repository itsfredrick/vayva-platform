  ...
  return (
    <div className="font-sans min-h-screen bg-yellow-400 text-black pb-24">
      ...
      <header className="px-4 py-4 bg-black text-yellow-400 border-b-4 border-white flex justify-between items-center sticky top-0 z-50 shadow-xl">
        <div className="font-heading font-black text-xl uppercase tracking-tighter transform -rotate-1 truncate max-w-[200px]">
          {businessName || "YAHAYA SUYA"}
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="bg-green-600 text-white px-3 py-1 font-bold text-xs uppercase tracking-widest animate-pulse border-2 border-white">
            Open
          </div>
        </div>
      </header>
      ...
      <div className="bg-yellow-400 p-6 text-center border-b-4 border-black">
        <h1 className="text-4xl font-black uppercase leading-none mb-4 transform -rotate-1">
          Best Suya In
          <br />
          Lagos!
        </h1>
        <p className="font-bold text-lg mb-6 border-2 border-black inline-block px-4 py-1 bg-white transform rotate-1">
          🌶️ Spicy Level: 100%
        </p>
      ...
      <div className="p-4 space-y-4 max-w-md mx-auto">
        {foodItems.map((item) => (
          ...
          >
            <div>
              <h3 className="font-black text-xl uppercase italic group-hover:underline">
                {item.name}
              </h3>
              <p className="font-bold text-gray-500 text-sm">
                {item.detail || "Delicious"}
              </p>
            </div>
