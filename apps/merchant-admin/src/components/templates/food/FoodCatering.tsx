  ...
  return (
    <div className="font-sans min-h-screen bg-orange-50/30 text-gray-900 pb-24">
      ...
      <header className="bg-white px-4 py-3 flex items-center justify-between sticky top-0 z-50 shadow-sm border-b border-orange-100/50">
        <div className="font-heading font-black text-xl tracking-tight text-orange-600 truncate max-w-[200px]">
          {businessName || "Mama's Kitchen"}
        </div>
        <div className="flex items-center gap-4">
          {itemCount > 0 && (
            <div
              className="bg-orange-100 text-orange-800 px-3 py-1 rounded-full text-xs font-bold"
              onClick={() => toggleCart()}
            >
      ...
      <div className="px-4 py-4 overflow-x-auto scrollbar-hide">
        <div className="flex gap-4 min-w-max">
          {[
            ...
            <div
              key={i}
              className={`flex flex-col items-center gap-2 ${i === 0 ? "opacity-100" : "opacity-60 grayscale"}`}
            >
              <div
                className={`w-16 h-16 rounded-2xl flex items-center justify-center text-3xl shadow-sm ${i === 0 ? "bg-orange-500 text-white shadow-orange-200" : "bg-white"}`}
              >
                {cat.icon}
              </div>
          ...
          {cateringItems.map((item) => (
            ...
            >
              <div className="w-28 h-28 bg-gray-100 rounded-xl overflow-hidden shrink-0 relative">
                <img
                  src={
                    "https://placehold.co/400x400/fff7ed/ea580c?text=" +
                    item.name.split(" ")[0]
                  }
                  alt={item.name}
