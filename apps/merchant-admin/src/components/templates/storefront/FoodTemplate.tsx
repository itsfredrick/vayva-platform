        ...
        <header className={cn("sticky top-[30px] z-40 backdrop-blur-md border-b flex-none transition-all", theme.bg + "/95", theme.border)}>
            <div className="max-w-2xl mx-auto px-6 h-16 flex items-center justify-between">
                <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate('home')}>
                    <img src="/logo-icon.png" alt="Vayva" className="w-10 h-10 object-contain" />
                    <span className="font-bold text-xl tracking-tight">CRAVE<span className="text-amber-500">.</span></span>
                </div>

                <div className="flex items-center gap-3">
                    <Button
                        size="sm"
                        className={cn("hidden md:flex rounded-full px-4 text-xs font-bold bg-amber-500 text-white hover:bg-amber-600 border-0")}
                        onClick={() => {/* Trigger WhatsApp Global */ }}
                    >
                        Order on WhatsApp
                    </Button>
...
const FoodProductModal = ({ config, openWhatsApp }: { config: StorefrontConfig, openWhatsApp: (p: string, m: string) => void }) => {
    const theme = getThemeStyles(config.theme);
    ...
    const toggleModifier = (modName: string, optLabel: string, type: 'single' | 'multiple') => {
        setModifiers(prev => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center pointer-events-none p-0 sm:p-4">
            {/* Backdrop */}
            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm pointer-events-auto transition-opacity" onClick={() => navigate('home')} />
            ...
            <div className="bg-white w-full max-w-lg h-[90vh] sm:h-auto sm:max-h-[85vh] rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col pointer-events-auto animate-in slide-in-from-bottom-10 duration-300 overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    ...
                    <p className="text-sm opacity-60 mb-6">{currentProduct.description}</p>
                        ...
                        {currentProduct.modifiers?.map(mod => (
                            <div key={mod.name}>
                                <div className="flex justify-between items-center mb-3">
                                    <h3 className="font-bold text-sm bg-gray-100 px-2 py-1 rounded-md">{mod.name}</h3>
                                    <span className="text-[10px] uppercase font-bold text-gray-400">{mod.type === 'single' ? 'Required' : 'Optional'}</span>
                                </div>
                                <div className="space-y-2">
                                    {mod.options.map(opt => {
                                        ...
                                        return (
                                            <div
                                                key={opt.label}
                                                onClick={() => toggleModifier(mod.name, opt.label, mod.type)}
                                                className={cn(
                                                    "flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all",
                                                    isSelected ? "border-amber-500 bg-amber-50" : "border-gray-100 hover:bg-gray-50"
                                                )}
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className={cn("w-4 h-4 rounded-full border flex items-center justify-center", isSelected ? "border-amber-500" : "border-gray-300")}>
                                                        {isSelected && <div className="w-2 h-2 rounded-full bg-amber-500" />}
                                                    </div>
                                                    <span className="text-sm font-medium">{opt.label}</span>
                                                </div>
                                                {opt.price > 0 && <span className="text-xs text-gray-500">+₦{opt.price}</span>}
                                            </div>
                                        );
                                    })}
                                </div>
                ...
                <div className="p-4 border-t bg-white safe-area-bottom">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-4 bg-gray-100 rounded-full px-2 py-1">
                            <button onClick={() => setQty(Math.max(1, qty - 1))} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-white transition-colors font-bold text-lg">-</button>
                            <span className="font-bold text-sm w-4 text-center">{qty}</span>
                            <button onClick={() => setQty(qty + 1)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-white transition-colors font-bold text-lg">+</button>
                        </div>
                        <span className="font-extrabold text-xl">₦{totalPrice.toLocaleString()}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <Button variant="outline" className="rounded-xl h-12 font-bold" onClick={handleWhatsAppBuy}>
                            WhatsApp
                        </Button>
                        <Button
            ...
            <section className="px-4 pt-8 pb-6 text-center">
                <div className="bg-gradient-to-br from-amber-600 to-amber-800 text-white rounded-3xl p-8 shadow-xl mb-4 relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <span className="text-[10px] font-bold tracking-widest uppercase bg-white/20 px-3 py-1 rounded-full mb-4 inline-block">Open till 9PM</span>
                        <h1 className="text-3xl md:text-4xl font-extrabold mb-3 leading-tight">{content.headline}</h1>
                        <p className="opacity-80 text-sm max-w-md mx-auto mb-6">{content.subtext}</p>
                        <Button
                    ...
                    {categories.map((c: any, i) => (
                        <button
                            ...
                            className={cn(
                                ...
                                i === 0 ? "bg-black text-white border-black" : "bg-gray-50 border-gray-200 text-gray-600 hover:bg-gray-100"
                            )}
                        >
                            {c}
                        </button>
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 py-1 flex flex-col">
                                            <div className="flex justify-between items-start mb-1">
                                                <h4 className="font-bold text-sm leading-tight group-hover:text-amber-600 transition-colors">{item.name}</h4>
                                                <span className="font-bold text-sm bg-gray-50 px-2 py-0.5 rounded-md">₦{item.price}</span>
                                            </div>
            ...
            <div className="px-4 space-y-4 pb-32">
                {cart.length === 0 ? (
                    <div className="text-center py-20 opacity-50">
                        <p>Tray is empty.</p>
                        <Button variant="link" onClick={() => navigate('home')}>View Menu</Button>
                    </div>
                ) : (
                    cart.map((item, i) => (
                        <div key={i} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex flex-col gap-3">
                            ...
                            {item.selectedModifiers && Object.keys(item.selectedModifiers).length > 0 && (
                                <div className="text-xs text-gray-500 bg-gray-50 p-2 rounded-lg">
                                    {Object.entries(item.selectedModifiers).map(([key, vals]) => (
                            ...
                            <div className="flex justify-between items-center pt-2">
                                <button onClick={() => removeFromCart(i)} className="text-xs text-red-500 font-bold">Remove</button>
                                <div className="flex items-center gap-3 bg-gray-50 p-1 rounded-lg border">
                                    <button onClick={() => updateQuantity(i, -1)} className="w-6 h-6 flex items-center justify-center bg-white rounded shadow-sm hover:bg-gray-100 font-bold">-</button>
...
const FoodCheckout = ({ config, onComplete }: { config: StorefrontConfig, onComplete: () => void }) => {
    ...
    const handlePlaceOrder = () => {
        ...
        KitchenService.addOrder({
            customerName: "New Customer", // Mocked
            source: 'website',
        ...
        <div className="min-h-screen bg-gray-50 animate-in slide-in-from-right-8 duration-300">
            ...
            <div className="px-4 space-y-6 pb-32">
                <section className="bg-white p-4 rounded-xl border shadow-sm space-y-4">
                    <h3 className="font-bold text-sm flex items-center gap-2"><Icon name="Truck" size={14} /> Fulfillment</h3>
                    <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="bg-blue-50 text-blue-800 text-xs p-3 rounded-lg flex gap-2">
                        ...
                        <span>Estimated Prep & Delivery: <strong>45-60 mins</strong></span>
                    </div>
                </section>

                <section className="bg-white p-4 rounded-xl border shadow-sm space-y-4">
                    <h3 className="font-bold text-sm"><Icon name="User" size={14} className="inline mr-2" /> Details</h3>
                    <Input placeholder="Full Name" className="bg-gray-50 border-0" />
                    <Input placeholder="Mobile Number" type="tel" className="bg-gray-50 border-0" />
                    <Input placeholder="Delivery Address" className="bg-gray-50 border-0" />
                </section>
            </div>
    ...
    return (
        <div id="storefront-container" className={cn("h-[800px] overflow-y-auto overflow-x-hidden flex flex-col relative bg-white transition-colors duration-300 scroll-smooth", theme.text, theme.font)}>
            ...
            <div className="flex-1 flex flex-col relative w-full">
                ...
                {route === 'product_detail' && <FoodProductModal config={config} openWhatsApp={openWhatsApp} />}
            ...
            {route === 'home' && (
                <footer className={cn("py-12 px-6 pb-24 text-center text-sm opacity-60 bg-gray-50 border-t", theme.border)}>
