...
export const PerfumeLuxuryTemplate: React.FC<TemplateProps> = ({ businessName, demoMode }) => {
    ...
    return (
        <div className="font-serif min-h-screen bg-[#0a0a0a] text-amber-50 selection:bg-amber-900 selection:text-white pb-20">
            ...
            <header className="py-8 px-8 flex justify-between items-center bg-[#0a0a0a]/80 backdrop-blur-md sticky top-0 z-50 border-b border-white/5">
                <div className="text-2xl tracking-[0.2em] uppercase">{businessName || "ELIXIR"}</div>
                <div className="flex gap-8 text-xs tracking-widest uppercase opacity-80 items-center">
                    <button className="hover:text-amber-400 transition-colors hidden md:block">Collection</button>
                    <button className="hover:text-amber-400 transition-colors hidden md:block">Our Story</button>
                    <button
                ...
                <div className="fixed inset-0 z-[100] bg-black/80 flex justify-end">
                    <div className="w-full max-w-sm bg-[#111] border-l border-white/10 h-full p-8 flex flex-col animate-in slide-in-from-right text-amber-50">
                        <div className="flex justify-between items-center mb-8 border-b border-white/10 pb-4">
                            <h2 className="text-xl tracking-[0.2em] uppercase">Your Selection</h2>
                            <button onClick={() => toggleCart(false)}><Icon name="X" size={24} /></button>
                        </div>
                        <div className="flex-1 overflow-auto space-y-4">
                            {itemCount === 0 ? <p className="text-white/30 text-center py-10 tracking-widest text-xs uppercase">Your cart is empty.</p> : (
                                <div className="p-4 border border-white/10 bg-white/5">
                                    <div className="flex justify-between items-center">
                                        <span className="font-light tracking-wide">{itemCount} items</span>
                                        <span className="text-xl text-amber-400 font-medium">{currency} {cartTotal.toLocaleString()}</span>
                                    </div>
                                </div>
                            )}
                        </div>
            ...
            <section className="relative h-[80vh] flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-10 text-center space-y-6 max-w-2xl px-4">
                    <p className="text-amber-400 tracking-[0.3em] text-xs uppercase">The Signature Collection</p>
                    <h1 className="text-5xl md:text-7xl font-light tracking-wide leading-tight">
                        Essence of <span className="italic font-serif text-amber-200">Royalty</span>
                    </h1>
                    ...
                    {perfumeItems.map((item) => (
                        <div key={item.id} className="group cursor-pointer">
                            <div className="aspect-[3/4] overflow-hidden bg-[#111] mb-6 relative">
                                <img src={item.img} alt={item.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000 opacity-80 group-hover:opacity-100" />
                                {demoMode && item.id === 'perf_1' && (
                                    <div className="absolute top-4 right-4 text-[10px] tracking-[0.2em] uppercase text-amber-400 border border-amber-400/30 px-3 py-1">Best Seller</div>
                                )}
                            </div>
                            <div className="text-center space-y-4">
                                <div>
                                    <h3 className="text-xl tracking-wide font-light">{item.name}</h3>
                                    <p className="text-xs text-amber-50/40 tracking-wider uppercase mt-1">{(item as any).notes}</p>
                                </div>

                                <div className="text-amber-400 font-medium text-lg">{currency} {item.price.toLocaleString()}</div>
            ...
            <section className="py-20 border-t border-white/5 bg-[#0f0f0f]">
                <div className="max-w-xl mx-auto text-center px-6">
                    <h2 className="text-2xl font-light mb-4 text-white">Unsure which scent defines you?</h2>
                    <p className="text-white/40 mb-8 font-light">Speak with our fragrance curators on WhatsApp for a personalized recommendation.</p>
                    <button
