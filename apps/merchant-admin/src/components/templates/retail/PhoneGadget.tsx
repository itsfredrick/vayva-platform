...
export const PhoneGadgetTemplate: React.FC<TemplateProps> = ({ businessName, demoMode }) => {
            ...
            <header className="bg-white border-b border-gray-200 sticky top-0 z-50">
                <div className="container mx-auto px-4 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-lg tracking-tight">{businessName || "Gadget Point"}</span>
                    </div>
                    <div className="flex items-center gap-4">
                        <button className="relative p-2 hover:bg-gray-100 rounded-full" onClick={() => toggleCart(true)}>
                            <Icon name="ShoppingBag" size={20} />
                            {itemCount > 0 && <span className="absolute top-0 right-0 w-4 h-4 bg-blue-600 text-white text-[10px] font-bold rounded-full flex items-center justify-center">{itemCount}</span>}
                        </button>
                    </div>
                </div>
            </header>
                ...
                <div className="fixed inset-0 z-[100] bg-black/50 flex justify-end">
                    <div className="w-full max-w-sm bg-white h-full p-6 flex flex-col animate-in slide-in-from-right">
                        ...
                        <div className="flex-1 overflow-auto">
                            {itemCount === 0 ? <p className="text-gray-500 text-center py-10">Your cart is empty.</p> : (
                                <div className="space-y-4">
                                    <div className="bg-blue-50 p-4 rounded text-sm text-blue-700">
                                        <p className="font-bold">Total: {currency} {cartTotal.toLocaleString()}</p>
                                    </div>
            ...
            <main className="container mx-auto px-4 py-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {gadgetItems.map((item) => (
                        <div key={item.id} className="bg-white p-3 rounded-xl border border-gray-200 flex flex-col shadow-sm">
                            ...
                            <div className="flex-1">
                                ...
                                {(item as any).specs && (
                                    <div className="flex flex-wrap gap-1 mb-2">
                                        {(item as any).specs.storage && <span className="text-[10px] bg-gray-100 px-1.5 py-0.5 rounded text-gray-600">{(item as any).specs.storage}</span>}
                                        {(item as any).specs.battery && <span className="text-[10px] bg-green-50 px-1.5 py-0.5 rounded text-green-700">🔋 {(item as any).specs.battery}</span>}
                                    </div>
                                )}
                                <div className="flex items-center justify-between mt-2">
                                    <span className="font-bold text-blue-600">{currency} {item.price.toLocaleString()}</span>
                                    <button
