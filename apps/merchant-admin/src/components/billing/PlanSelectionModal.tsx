...
const PlanOption = ({ name, price, features, recommended, current, onSelect, disabled }: any) => (
    <div className={`p-6 rounded-2xl border ${recommended ? 'border-primary-500 bg-primary-50/50 ring-2 ring-primary-200' : 'border-gray-200'} relative flex flex-col`}>
        {recommended && (
        ...
        <ul className="space-y-3 mb-6 flex-1">
            {features.map((f: string, i: number) => (
                <li key={i} className="flex items-center gap-2 text-sm text-gray-600">
                    <Icon name={"CheckCircle" as any} size={16} className="text-green-600 flex-shrink-0" />
                    {f}
                </li>
            ))}
        </ul>
...
export const PlanSelectionModal = ({ isOpen, onClose, onSelectPlan, currentPlan, processing }: PlanSelectionModalProps) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl animate-in zoom-in-95 duration-200 relative p-8">
                ...
                <div className="text-center mb-10">
                    <h2 className="text-3xl font-bold text-gray-900 mb-2">Choose the perfect plan</h2>
                    <p className="text-gray-500 text-lg">Scale your business with powerful tools and insights.</p>
                </div>
