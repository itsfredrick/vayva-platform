                        ...
                        <div key={index} className="relative">
                            <div className="flex items-center gap-4 mb-2">
                                <div className={cn(
                                    ...
                                    index === 0 ? "border-gray-200 text-gray-400" :
                                        index === funnel.steps.length - 1 ? "border-green-500 text-green-600 bg-green-50" :
                                ...
                                <div className="flex-1">
                                    <div className="flex justify-between items-center mb-1">
                                        ...
                                        <span className="text-sm font-mono font-medium text-gray-600">{step.count.toLocaleString()}</span>
                                    </div>
                                    <div className="h-2 bg-gray-100 rounded-full overflow-hidden w-full">
                                        <div
                                            className={cn("h-full rounded-full transition-all duration-1000",
                                                index === funnel.steps.length - 1 ? "bg-green-500" : "bg-indigo-500"
                                            )}
                                            style={{ width: `${widthPercent}%` }}
                                        />
                                    </div>
                                    {step.dropoff > 0 && (
                                        <p className="text-xs text-red-400 mt-1 font-medium">{step.dropoff}% drop-off</p>
                                    )}
                                </div>
                            </div>
