...
import { WhatsAppMessage, WhatsAppMessageSender, WhatsAppConversation } from '@vayva/shared';
...
export const ChatWindow = ({ conversation, messages, onSendMessage, isLoadingMessages }: ChatWindowProps) => {
    ...
    return (
        <div className="flex flex-col h-full bg-[#E5DDD5]/30 relative"> {/* WhatsApp-ish mild background */}
            ...
            <header className="h-[60px] bg-white border-b border-gray-200 flex items-center px-4 justify-between shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="font-bold text-sm text-gray-900">{conversation.customerName || conversation.customerPhone}</h3>
                        ...
                        return (
                            <div key={msg.id} className="flex justify-center my-4">
                                <div className="bg-gray-100 text-gray-500 text-xs px-3 py-1.5 rounded-full shadow-sm border border-gray-200 flex items-center gap-2">
                                    ...
                                    <span className="font-medium">Automated by Vayva:</span> {msg.content}
                                </div>
                            </div>
                        );
                    }
                    ...
                    return (
                        <div key={msg.id} className={cn("flex w-full", isMe ? "justify-end" : "justify-start")}>
                            <div className={cn(
                                ...
                                isMe ? "bg-[#D9FDD3] text-gray-900 rounded-br-none" : "bg-white text-gray-900 rounded-bl-none"
                            )}>
                                <p className="leading-relaxed">{msg.content}</p>
                                <div className={cn("text-[10px] mt-1 flex items-center gap-1", isMe ? "justify-end text-green-800/60" : "text-gray-400")}>
            ...
            <div className="p-3 bg-white flex items-end gap-2 shrink-0">
                ...
                >
                    <Icon name="Send" size={18} className={inputValue.trim() ? "translate-x-0.5" : ""} />
                </button>
            </div>
        </div>
    );
