...
export const PlanComparisonMobile = () => {
    const [selectedPlan, setSelectedPlan] = useState<'free' | 'growth' | 'pro'>('growth');
        ...
        <div className="space-y-8">
            ...
            <div className="flex p-1 bg-gray-100 rounded-xl">
                {PLANS.map(plan => (
                    ...
                    >
                        {plan.key === 'free' ? 'Free' : plan.monthlyAmount === 30000 ? '₦30k' : '₦40k'}
                    </button>
                ))}
            </div>
            ...
            <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-xl">
                ...
                <div className="space-y-6">
                    {[
                        { name: 'Monthly Orders', val: selectedPlan === 'free' ? '100' : selectedPlan === 'growth' ? '1,000' : 'Unlimited' },
                        { name: 'Products / SKUs', val: selectedPlan === 'free' ? '50' : selectedPlan === 'growth' ? '500' : 'Unlimited' },
                        { name: 'Team Seats', val: selectedPlan === 'pro' ? '5' : '1' },
                        { name: 'Blueprint Templates', val: selectedPlan === 'free' ? 'Basic' : 'All' },
                        ...
                        <div key={feat.name} className="flex justify-between items-center py-4 border-b border-gray-50 last:border-0">
                            <span className="text-sm font-bold text-gray-400 uppercase tracking-widest">{feat.name}</span>
