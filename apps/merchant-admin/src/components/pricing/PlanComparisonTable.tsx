            ...
            <table className="w-full text-left border-collapse">
                <thead>
                    <tr className="bg-gray-50 border-b border-gray-100">
                        <th className="p-8 text-sm font-black text-gray-400 uppercase tracking-widest w-1/4">Features</th>
                        {PLANS.map(plan => (
                            <th key={plan.key} className={`p-8 text-xl font-black text-[#0F172A] text-center ${plan.featured ? 'bg-green-50/30' : ''}`}>
                                {plan.name}
                            </th>
                        ))}
                    </tr>
