...
export function AddOnCard({
  ...
  return (
    <div
      className={`bg-white border rounded-lg p-6 ${!compatibility.isCompatible ? "opacity-60" : ""}`}
    >
      {/* Header */}
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <div className="flex gap-2">
            <span
              className={`text-xs px-2 py-1 rounded ${getTypeColor(addon.type)}`}
            >
              {addon.type}
            </span>
            <span
              className={`text-xs px-2 py-1 rounded ${getPlanBadge(addon.requiredPlan)}`}
            >
              {addon.requiredPlan}
            </span>
          </div>
      ...
      {/* Description */}
      <p className="text-sm text-[#64748B] mb-4">{addon.description}</p>
