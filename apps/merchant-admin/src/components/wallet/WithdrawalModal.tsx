...
import { WithdrawalEligibility, WithdrawalQuote, PayoutAccount } from '@vayva/shared';
...
export const WithdrawalModal = ({ isOpen, onClose, availableBalance, onSuccess }: WithdrawalModalProps) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Withdraw Funds" className="max-w-lg">
            <div className="space-y-6">
                ...
                {step === 1 && (
                    <div className="space-y-4">
                        {loading && <div className="text-gray-500 text-sm">Checking eligibility...</div>}
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        {eligibility.kycStatus === 'verified' ? (
                                            ...
                                            <span className="text-xs font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded-md flex items-center gap-1">
                                                <Icon name={"AlertTriangle" as any} size={12} /> Action Required
                                            </span>
                                        )}
                                    </div>
                                    <div className="flex items-center justify-between">
                                        ...
                                        {eligibility.hasPayoutAccount ? (
                                            <span className="text-xs font-bold text-green-600 bg-green-50 px-2 py-1 rounded-md">Added</span>
                                        ) : (
                                            <span className="text-xs font-bold text-red-600 bg-red-50 px-2 py-1 rounded-md">Missing</span>
                                        )}
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm text-gray-600">Available Balance</span>
                                        <span className="font-mono text-sm font-bold">₦{availableBalance.toLocaleString()}</span>
                                    </div>
                                </div>
                                ...
                                {eligibility.isEligible ? (
                                    ...
                                    <div className="text-center">
                                        <p className="text-sm text-red-600 font-medium mb-3">
                                            {eligibility.blockedReasons[0] || "Please resolve the issues above."}
                                        </p>
                ...
                {step === 2 && (
                    <div className="space-y-6">
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-gray-500 uppercase">Payout Account</label>
                            {accounts.map(acc => (
                                <div
                                    key={acc.id}
                                    ...
                                    className={cn(
                                        ...
                                        selectedAccount === acc.id ? "border-black bg-gray-50 shadow-sm" : "border-gray-200"
                                    )}
                                >
                                    <div>
                                        <div className="font-bold text-sm text-gray-900">{acc.bankName}</div>
                                        <div className="text-xs text-gray-500">{acc.accountName} • {acc.accountNumber}</div>
                                    </div>
                                    {selectedAccount === acc.id && <Icon name="Check" size={16} className="text-black" />}
                                </div>
                            ))}
                            {/* Placeholder for Add Functionality */}
                            <button className="text-xs font-bold text-black underline pl-1 pt-1">+ Add another account</button>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex justify-between items-center text-xs text-gray-500 px-1">
                                ...
                                <button onClick={() => setAmount(availableBalance)} className="font-bold text-blue-600 hover:underline">Max</button>
                            </div>
                        </div>
                    ...
                    <div className="space-y-6">
                        <div className="bg-gray-50 border border-gray-100 rounded-xl p-5 space-y-3">
                            <div className="flex justify-between text-sm">
                                ...
                                <span className="font-bold font-mono">₦{quote.amount.toLocaleString()}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-gray-500">Service Fee</span>
                                <span className="font-bold font-mono text-gray-600">- ₦{quote.fee.toLocaleString()}</span>
                            </div>
                            <div className="h-px bg-gray-200 my-2" />
                            <div className="flex justify-between text-base">
                                <span className="font-bold text-gray-900">Total to You</span>
                                <span className="font-bold font-mono text-green-600">₦{quote.netAmount.toLocaleString()}</span>
                            </div>
                        </div>
                        ...
                        <div className="flex items-start gap-2 bg-blue-50 text-blue-800 p-4 rounded-xl text-xs">
                            ...
                            <p>Funds will arrive in your <strong>{accounts.find(a => a.id === selectedAccount)?.bankName}</strong> account {quote.estimatedArrival}.</p>
                        </div>
