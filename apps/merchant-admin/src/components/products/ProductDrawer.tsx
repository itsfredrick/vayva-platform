...
import { ProductServiceType, ProductServiceStatus, ProductServiceItem } from '@vayva/shared';
import { Button, Icon, Drawer, Input } from '@vayva/ui';

// --- Local UI Components (Missing in Shared) ---

const Label = ({ children, className }: { children: React.ReactNode, className?: string }) => (
    <label className={`block text-xs font-bold text-gray-700 mb-1.5 uppercase tracking-wide ${className}`}>{children}</label>
);
...
const Textarea = forwardRef<HTMLTextAreaElement, React.ComponentProps<'textarea'>>((props, ref) => (
    <div className="relative">
        <textarea
            ...
            {...props}
        />
        {props['aria-invalid'] && <div className="text-red-500 text-xs mt-1">{props['aria-errormessage']}</div>}
    </div>
));
...
const ImageUploader = ({ images, onChange }: { images: string[], onChange: (images: string[]) => void }) => {
        ...
        <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-3">
                ...
                >
                    ...
                    <span className="text-[10px] uppercase font-bold text-gray-400">Add Image</span>
                </button>
                <input
                    ref={fileInputRef}
                    type="file"
...
export const ProductDrawer: React.FC<ProductDrawerProps> = ({ isOpen, onClose, onSubmit, initialData, isLoading }) => {
    const { register, handleSubmit, watch, setValue, reset, formState: { errors, isSubmitting } } = useForm<ProductFormValues>({
        ...
        defaultValues: {
            ...
            availability: { days: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'], timeRange: '09:00 - 17:00' },
            images: []
        }
    });
    ...
    useEffect(() => {
        if (isOpen && initialData) {
            ...
            reset({
                ...
                availability: initialData.availability || { days: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'], timeRange: '09:00 - 17:00' },
    ...
    return (
        <Drawer isOpen={isOpen} onClose={onClose} title={initialData ? "Edit Product" : "New Product"}>
            <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-8 pb-20">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <Select {...register('type')}>
                                <option value={ProductServiceType.RETAIL}>Physical Product</option>
                        ...
                        <div>
                            ...
                            <Input {...register('category')} placeholder="e.g. Summer Collection" />
                        </div>
                    </div>
                </section>
                ...
                <section className="space-y-4">
                    <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wide border-b border-gray-100 pb-2">
                        {selectedType === ProductServiceType.SERVICE ? 'Service Details' : 'Pricing & Inventory'}
                    </h3>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                            ...
                            <>
                                <div className="col-span-2 flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100">
                                    <div className="text-sm">
                                        <div className="font-bold text-gray-900">Track Inventory</div>
                                        <div className="text-gray-500">Auto-update stock on new orders</div>
                                    </div>
                                    <Switch checked={inventoryEnabled || false} onCheckedChange={(c) => setValue('inventory.enabled', c)} />
                                </div>
                                ...
                                {inventoryEnabled && (
                                    <>
                                        <div>
                                            ...
                                            <Input type="number" {...register('inventory.quantity')} placeholder="0" />
                                        </div>
                                        <div>
                                            <Label>Low Stock Alert</Label>
                                            <Input type="number" {...register('inventory.lowStockThreshold')} placeholder="5" />
                                        </div>
                                    </>
                                )}
                            </>
                        ...
                        {selectedType === ProductServiceType.FOOD && (
                            <div className="col-span-2 flex items-center justify-between p-3 bg-orange-50 rounded-lg border border-orange-100">
                                <div className="text-sm">
                                    <div className="font-bold text-orange-900">Today's Special</div>
                                    <div className="text-orange-700">Highlight this item on your menu</div>
                                </div>
                                <Switch
                        ...
                        {selectedType === ProductServiceType.SERVICE && (
                            <div className="col-span-2 space-y-4">
                                <div>
                                    ...
                                    <Input {...register('availability.days')} placeholder="Mon, Tue, Wed..." />
                                </div>
                                <div>
                                    <Label>Time Range</Label>
                                    <Input {...register('availability.timeRange')} placeholder="09:00 - 17:00" />
                                </div>
                            </div>
                        )}
                    </div>
