...
export const ProductList: React.FC<ProductListProps> = ({ items, onEdit, onDelete, onCreate, isLoading }) => {
    if (isLoading) {
    ...
    if (items.length === 0) {
        return (
            <div className="p-16 text-center text-gray-500 flex flex-col items-center">
                ...
                <h3 className="text-xl font-bold text-gray-900 mb-2">Your product catalog is empty</h3>
                <p className="text-gray-500 max-w-sm mb-8">
                    Add products so Vayva can generate invoices, track inventory, and help customers order online.
                </p>
    ...
    return (
        <>
            ...
            <div className="md:hidden space-y-4">
                {items.map((item) => (
                    <div key={item.id} className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm active:scale-[0.99] transition-transform" onClick={() => onEdit(item)}>
                        <div className="flex items-start gap-4 mb-3">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="text-xs text-gray-500 line-clamp-2 mt-0.5 mb-2">{item.description || 'No description'}</div>
