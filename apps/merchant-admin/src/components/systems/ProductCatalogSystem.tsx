...
export function ProductCatalogSystem({ onComplete }: { onComplete: () => void }) {
            ...
            <div className="flex-1 overflow-y-auto pr-2">
                        ...
                        {['create', 'variants', 'pricing'].map((s, idx) => (
                            <div
                                ...
                                className={cn(
                                    ...
                                    (step === s || (step === 'pricing' && s !== 'pricing') || (step === 'variants' && s === 'create'))
                                        ? "bg-black"
                                        : "bg-gray-100"
                                )}
                            />
                ...
                {step === 'create' && (
                    <div className="space-y-4 animate-in slide-in-from-right-4 fade-in">
                                ...
                                {(['physical', 'digital', 'service'] as const).map(t => (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            product.type === t ? "bg-black text-white border-black" : "bg-white text-gray-500 border-gray-200"
                                        )}
                                    >
                                        {t}
                                    </button>
                        ...
                        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100">
                            <div>
                                <span className="font-bold text-sm block text-gray-900">This product has options</span>
                                <span className="text-xs text-gray-500">Like size, color, or material</span>
                            </div>
                            <Switch
                                checked={product.hasVariants}
                                onCheckedChange={(c) => setProduct({ ...product, hasVariants: c })}
                            />
                        </div>
                    </div>
                )}
                ...
                {step === 'variants' && (
                    <div className="space-y-6 animate-in slide-in-from-right-4 fade-in">
                        {product.variants.map((variant, idx) => (
                            <div key={idx} className="p-4 bg-gray-50 rounded-xl border border-gray-200 space-y-3">
                                <div className="flex justify-between">
                                    <label className="text-xs font-bold text-gray-500 uppercase">Option Name</label>
                                ...
                                />
                                <div>
                                    <label className="text-xs font-bold text-gray-500 uppercase mb-1 block">Values (comma separated)</label>
                                    <Input
                                        placeholder="Small, Medium, Large"
                                        value={variant.options.join(', ')}
                                        onChange={(e) => {
                                            const newV = [...product.variants];
                                            newV[idx].options = e.target.value.split(',').map(s => s.trim());
                                            setProduct({ ...product, variants: newV });
                                        }}
                                    />
                                </div>
                        ...
                        <Button variant="outline" className="w-full border-dashed border-gray-300 text-gray-500" onClick={() => {
                            setProduct({ ...product, variants: [...product.variants, { group: 'New Option', options: [] }] })
                        }}>
                            + Add another option
                        </Button>
                    </div>
                ...
                {step === 'pricing' && (
                    <div className="space-y-6 animate-in slide-in-from-right-4 fade-in">
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-gray-500 uppercase">Selling Price <span className="text-red-500">*</span></label>
                        ...
                        <div className="p-4 bg-green-50 rounded-xl border border-green-100 flex items-start gap-3">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-xs text-green-700">
                                    When customers ask "How much?", our AI will reply: <br /> "It's ₦{product.basePrice || '...'} per unit."
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-white rounded-2xl shadow-xl w-full overflow-hidden max-w-[260px] transform transition-all hover:scale-105 duration-300">
                    <div className="aspect-square bg-gray-200 flex items-center justify-center relative">
                        ...
                        {product.type === 'digital' && (
                            <div className="absolute top-2 right-2 bg-black/50 text-white text-[10px] px-2 py-1 rounded backdrop-blur-sm">DIGITAL</div>
                        )}
                    </div>
                    <div className="p-4 space-y-2">
                        <div className="h-4 w-3/4 bg-gray-100 rounded animate-pulse" style={{ display: product.name ? 'none' : 'block' }} />
                        {product.name && <h3 className="font-bold text-gray-900 leading-tight">{product.name}</h3>}

                        <div className="flex items-baseline gap-1">
                            <span className="text-xs text-gray-500">₦</span>
                            <span className="text-lg font-bold text-black">{product.basePrice || '0.00'}</span>
                        </div>

                        {product.hasVariants && (
                            <div className="flex gap-1 flex-wrap pt-2">
                                {product.variants[0]?.options.slice(0, 3).map(opt => (
                                    <span key={opt} className="text-[10px] border border-gray-200 px-1.5 py-0.5 rounded text-gray-600">
                                        {opt}
                                    </span>
                                ))}
                                {product.variants[0]?.options.length > 3 && <span className="text-[10px] text-gray-400">+More</span>}
                            </div>
                        )}
                    </div>
                </div>
