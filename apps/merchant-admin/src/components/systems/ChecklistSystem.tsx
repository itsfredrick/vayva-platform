    ...
    if (collapsed || dismissed) {
        return (
            <div className="fixed bottom-6 right-6 z-40">
                <button
                    onClick={() => { setCollapsed(false); setDismissed(false); }}
            ...
            <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
                ...
                <div className="bg-black text-white p-4 flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={() => setCollapsed(true)} className="hover:bg-white/20 p-1 rounded transition-colors">
                            <Icon name="Minimize2" size={14} />
                        </button>
                        <button onClick={() => setDismissed(true)} className="hover:bg-white/20 p-1 rounded transition-colors">
                            <Icon name="X" size={14} />
                        </button>
                    </div>
                </div>
                    ...
                    {items.sort((a, b) => b.priority - a.priority).map((item) => (
                        <div
                            ...
                            className={cn(
                                ...
                                item.status === 'completed' ? "bg-gray-50 opacity-60" : "bg-white hover:bg-gray-50"
                            )}
                        >
                            <div className={cn(
                                "w-5 h-5 rounded-full border flex items-center justify-center shrink-0 mt-0.5",
                                item.status === 'completed' ? "bg-green-500 border-green-500 text-white" : "border-gray-300 text-transparent"
                            )}>
                                <Icon name="Check" size={10} />
                            </div>

                            <div className="flex-1">
                                <h4 className={cn("text-sm font-bold text-gray-900", item.status === 'completed' && "line-through text-gray-500")}>
                                    {item.title}
                                </h4>
