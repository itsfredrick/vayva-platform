...
export const SupportChat: React.FC = () => {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { id: '1', role: 'bot', text: 'Hi! I am your Vayva Support Assistant. How can I help you manage your store today?', timestamp: new Date() }
    ]);
    ...
    const handleSendMessage = async () => {
        ...
        try {
            const response = await fetch('/api/support/chat', {
                ...
                body: JSON.stringify({ query: input, history: messages.map(m => ({ role: m.role === 'bot' ? 'assistant' : 'user', content: m.text })) })
            });
            ...
            const botMsg: Message = {
                ...
                role: 'bot',
                text: data.message || "I'm sorry, I'm having trouble processing that right now.",
                timestamp: new Date(),
            ...
            setMessages(prev => [...prev, { id: 'err', role: 'bot', text: 'Sorry, I lost my connection. Please try again.', timestamp: new Date() }]);
        } finally {
            setIsLoading(false);
        }
    };

    const handleFeedback = async (msgLocalId: string, serverId: string, rating: 'SOLVED' | 'NOT_SOLVED') => {
        setMessages(prev => prev.map(m => m.id === msgLocalId ? { ...m, feedback: rating } : m));
        try {
            await fetch('/api/support/feedback', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ messageId: serverId, rating, conversationId: 'session_1' })
            });
        } catch (e) {
            ...
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span className="text-[10px] text-indigo-100">Usually replies instantly</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        {messages.filter(m => m.feedback === 'NOT_SOLVED').length >= 2 && (
                            <div className="bg-orange-50 p-3 border-b border-orange-100 flex items-center justify-between">
                                <div className="text-xs text-orange-800">
                                    <strong>Need extra help?</strong><br />
                                    We can connect you to a human agent.
                                </div>
                                <button
                                    onClick={() => { setInput('Please connect me to a human agent'); handleSendMessage(); }}
                                ...
                                <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                                    ...
                                    {msg.role === 'bot' && (msg as any).actions && (
                                        <div className="mt-2 flex flex-wrap gap-2">
                                            {(msg as any).actions.map((action: string, i: number) => (
                                                <button
                                                    key={i}
                                                    onClick={() => setInput(action === 'Talk to Human' ? 'I need to speak to a human' : action)}
                                    ...
                                    {msg.role === 'bot' && msg.messageId && (
                                        <div className="mt-1 flex gap-2 ml-1">
                                            {msg.feedback ? (
                                                <span className={`text-[10px] font-medium ${msg.feedback === 'SOLVED' ? 'text-green-600' : 'text-gray-400'}`}>
                                                    {msg.feedback === 'SOLVED' ? 'Marked as Solved' : 'Feedback recorded'}
                                                </span>
                                            ) : (
                                                <>
                                                    <button
                                                        onClick={() => handleFeedback(msg.id, msg.messageId!, 'SOLVED')}
                                                        className="text-gray-400 hover:text-green-600 transition-colors" title="Solved my issue">
                                                        <ThumbsUp size={12} />
                                                    </button>
                                                    <button
                                                        onClick={() => handleFeedback(msg.id, msg.messageId!, 'NOT_SOLVED')}
                                                        className="text-gray-400 hover:text-red-500 transition-colors" title="Didn't help">
                                                        <ThumbsDown size={12} />
                                                    </button>
                                                </>
                                            )}
                        ...
                        <div className="bg-white border-t border-gray-100">
                            ...
                            <div className="px-4 py-2 flex gap-2 overflow-x-auto no-scrollbar mask-linear-fade">
                                {['Check Status', 'Billing Help', 'Connect WhatsApp', 'Talk to Human'].map(chip => (
                                    <button
                                        key={chip}
                                        onClick={() => {
