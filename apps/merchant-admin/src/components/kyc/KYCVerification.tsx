        ...
        return (
            <div className="text-center py-12 space-y-6">
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-text-secondary">Your identity has been verified system-wide.</p>
                </div>
                <Button onClick={onCancel} className="w-full">Return to Dashboard</Button>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-md mx-auto space-y-8 p-4">
            <div className="space-y-2">
                ...
                <p className="text-text-secondary font-medium">Choose a method to verify your business identity.</p>
            </div>

            {step === 'SELECT' ? (
                <div className="grid grid-cols-1 gap-4">
                    <button
                        onClick={() => { setMethod('BVN'); setStep('FORM'); }}
                    ...
                    >
                        ...
                        <div>
                            <h3 className="font-bold text-white">Bank Verification Number (BVN)</h3>
                            <p className="text-xs text-text-secondary">Instant verification via your 11-digit BVN.</p>
                        </div>
                    </button>

                    <button
                        onClick={() => { setMethod('NIN'); setStep('FORM'); }}
                        className="p-6 bg-white/5 border border-white/10 rounded-2xl flex items-center gap-4 hover:bg-white/10 transition-all text-left group"
                    >
                        ...
                        <div>
                            <h3 className="font-bold text-white">National Identity Number (NIN)</h3>
                            <p className="text-xs text-text-secondary">Verify using your 11-digit National ID.</p>
                        </div>
                    </button>
                ...
                <form onSubmit={handleSubmit} className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                    ...
                    <div className="flex items-start gap-3 p-4 bg-white/5 rounded-xl border border-white/10">
                        <input
                            ...
                            id="consent-check"
                        />
                        <label htmlFor="consent-check" className="text-xs text-text-secondary leading-relaxed cursor-pointer select-none">
                            I hereby give consent for Vayva to verify my identity details against official government databases as provided by authorized providers.
                        </label>
                    </div>
                    ...
                    <div className="flex gap-3 pt-4">
                        ...
                        <Button type="submit" isLoading={loading} className="flex-[2]">Verify Identity</Button>
                    </div>
                </form>
            )}
            {/* Partner Attribution */}
            <div className="flex items-center justify-center gap-2 pt-4 opacity-50 grayscale hover:grayscale-0 transition-all">
                <span className="text-[10px] uppercase font-bold text-text-secondary tracking-widest">Powered by</span>
