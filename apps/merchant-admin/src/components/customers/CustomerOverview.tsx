...
export const CustomerOverview = ({ customers }: CustomerOverviewProps) => {
    ...
    const returning = customers.filter(c => c.status === CustomerStatus.RETURNING || c.status === CustomerStatus.VIP).length;
    const newCustomers = customers.filter(c => c.status === CustomerStatus.NEW).length;
    // Mock sort by spend for 'Top'
            ...
            {topCustomer && (
                <div className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-3 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                    ...
                    <div>
                        ...
                        <p className="font-bold text-sm text-gray-900 truncate max-w-[120px]">{topCustomer.name}</p>
                    </div>
                </div>
            )}
        </div>
