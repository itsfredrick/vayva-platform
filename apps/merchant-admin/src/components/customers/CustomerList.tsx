...
export const CustomerList = ({ customers, isLoading, onSelectCustomer }: CustomerListProps) => {
                        ...
                        >
                            ...
                            <td className="md:px-6 md:py-4 flex items-center gap-3 mb-2 md:mb-0">
                                <div className={cn(
                                    ...
                                    customer.status === 'vip' ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-700"
                                )}>
                                    {customer.name.charAt(0)}
                                </div>
                                <div>
                                    <div className="font-bold text-gray-900 text-sm md:text-base">{customer.name}</div>
                            ...
                            <td className="md:px-6 md:py-4 mb-2 md:mb-0 flex md:table-cell">
                                <span className={cn(
                                    ...
                                    customer.status === CustomerStatus.VIP ? "bg-amber-100 text-amber-700" :
                                        customer.status === CustomerStatus.NEW ? "bg-blue-100 text-blue-700" :
                                            "bg-gray-100 text-gray-600"
                                )}>
                                    {customer.status === CustomerStatus.VIP && <Icon name="Crown" size={10} />}
                                    {customer.status}
                                </span>
                            </td>
                            ...
                            <td className="md:hidden text-xs text-gray-500 mb-4 flex gap-4">
                                ...
                                <span className="font-bold text-gray-900">{formatCurrency(customer.totalSpend)}</span>
                            </td>
                            ...
                            <td className="md:px-6 md:py-4 text-right md:table-cell" onClick={(e) => e.stopPropagation()}>
                                <div className="flex items-center justify-end gap-2 md:opacity-0 group-hover:opacity-100 transition-opacity">
                                    <WhatsAppAction
                                        ...
                                        variant="icon"
                                    />
                                    <Button size="sm" variant="ghost" className="text-gray-400 hover:text-gray-900" onClick={() => onSelectCustomer(customer)}>
                                        <Icon name="ChevronRight" size={16} />
                                    </Button>
                                </div>
                                {/* Mobile Action: Absolute positioned */}
                                <div className="absolute top-4 right-4 md:hidden">
                                    <WhatsAppAction phone={customer.phone} name={customer.name} variant="icon" />
                                </div>
                            </td>
                        </tr>
                    ))}
