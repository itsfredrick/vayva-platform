...
export class NotificationManager {
     ...
     */
    static async trigger(storeId: string, type: NotificationType, variables: Record<string, any> = {}) {
        ...
        const activeChannels = (prefs?.channels as any) || { email: true, banner: true, in_app: true, whatsapp: false };
        const activeCategories = (prefs?.categories as any) || { orders: true, system: true, account: true, payments: true };
        ...
        for (const channel of channelsToNotify) {
            ...
            const outbox = await (prisma as any).notificationOutbox.create({
                data: {
                    ...
                    channel,
                    to: recipient,
                    payload: { title, body, ctaLabel: metadata.ctaLabel, ctaLink: metadata.ctaLink },
                    status: 'QUEUED'
                }
            });
            ...
            if (channel === 'WHATSAPP') {
                console.log(`[NotificationManager] [STUB] Sending WhatsApp to ${recipient}: ${body}`);
            }
        }
    ...
    private static async getRecipientInfo(storeId: string, channel: string): Promise<string | null> {
        if (channel === 'WHATSAPP') {
