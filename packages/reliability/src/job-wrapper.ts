...
export class JobWrapper {
    ...
    static classifyError(error: any): 'transient' | 'permanent' {
        ...
        if (error.code === 'ETIMEDOUT' || error.code === 'ECONNRESET') return 'transient';
        if (error.response?.status === 429 || error.response?.status >= 500) return 'transient';

        // Invalid credentials, 4xx = permanent
        if (error.response?.status >= 400 && error.response?.status < 500) return 'permanent';
