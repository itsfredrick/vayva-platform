...
export const LegalContentRenderer: React.FC<LegalContentRendererProps> = ({ document: doc, className, showTitle = false }) => {
    return (
        <div className={cn("space-y-16 max-w-none prose prose-slate", className)}>
            {showTitle && (
                <header className="mb-20">
                    <h1 className="text-4xl md:text-5xl font-bold text-black mb-6 tracking-tight" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
                        {doc.title}
                    </h1>
...
const SectionRenderer = ({ section, id }: { section: LegalSection; id: string }) => {
    ...
    return (
        <section
            ...
            className={cn(
                ...
                section.type === 'callout-important' ? "bg-[#F7F7F7] border-[#E6E6E6]" : "",
                section.type === 'callout-nigeria' ? "bg-green-50/50 border-green-100" : ""
            )}
        >
            {section.heading && (
            ...
            <div className={cn(
                ...
                {section.content.map((paragraph, idx) => {
                    ...
                    if (isDefinitions && paragraph.includes(':')) {
                        ...
                        return (
                            <div key={idx} className="p-5 bg-white border border-gray-100 rounded-xl shadow-sm hover:shadow-md transition-all">
                                <strong className="block text-black text-sm uppercase tracking-wider mb-2" dangerouslySetInnerHTML={{ __html: parseMarkdown(term) }} />
                                <p className="text-sm leading-relaxed text-gray-500 m-0" dangerouslySetInnerHTML={{ __html: parseMarkdown(def) }} />
                            </div>
                        );
                    }
                ...
                {isDefinitions && section.list && section.list.map((item, idx) => {
                    if (item.includes(':')) {
                        const [term, ...defParts] = item.split(':');
                        const def = defParts.join(':');
                        return (
                            <div key={`list-def-${idx}`} className="p-5 bg-white border border-gray-100 rounded-xl shadow-sm hover:shadow-md transition-all">
                                <strong className="block text-black text-sm uppercase tracking-wider mb-2" dangerouslySetInnerHTML={{ __html: parseMarkdown(term) }} />
                                <p className="text-sm leading-relaxed text-gray-500 m-0" dangerouslySetInnerHTML={{ __html: parseMarkdown(def) }} />
                            </div>
                        );
            ...
            {section.list && !isDefinitions && (
                <ul className="mt-8 space-y-4 list-none p-0">
                    {section.list.map((item, idx) => (
                        <li key={idx} className="flex gap-4 text-[17px] md:text-[18px] leading-[1.8] text-gray-700">
