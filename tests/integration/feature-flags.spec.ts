...
test.describe('Feature Flags & Kill Switches', () => {
    ...
    test('Global Disabled', async () => {
        const enabled = await FlagService.isEnabled(KEY, { merchantId: 'random_merch' });
        expect(enabled).toBe(false);
    });

    test('Allowlist Override', async () => {
        const enabled = await FlagService.isEnabled(KEY, { merchantId: 'allowed_merch_1' });
    ...
    test('Blocklist Prevents', async () => {
        ...
        const enabled = await FlagService.isEnabled(KEY, { merchantId: 'blocked_merch_1' });
        expect(enabled).toBe(false);

        const other = await FlagService.isEnabled(KEY, { merchantId: 'other_merch' });
