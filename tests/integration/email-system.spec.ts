...
test.describe('Email System', () => {
    ...
    test('Team Invite Integration', async () => {
        ...
        const email = 'invitee@vayva.com';

        await TeamService.inviteMember(merchantId, 'admin_u', { email, role: ROLES.VIEWER });
    ...
    test('Suppression Check', async () => {
        const blockedEmail = 'blocked@vayva.com';
        ...
        const msg = await prisma.emailMessage.findFirst({ where: { correlationId: corrId } });
