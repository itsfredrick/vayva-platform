...
test.describe("Status Ops & Uptime", () => {
  test("Uptime Check & Incident Automation", async () => {
    ...
    let incidents = await prisma.statusIncident.findMany({
      include: { links: true },
    });
    let related = incidents.filter((i) =>
      i.links.some((l) => l.uptimeCheckId === check.id),
    );
    ...
    incidents = await prisma.statusIncident.findMany({
      include: { links: true },
    });
    related = incidents.filter((i) =>
      i.links.some((l) => l.uptimeCheckId === check.id),
    );
