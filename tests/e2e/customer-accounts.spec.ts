...
test.describe("Customer Accounts", () => {
  ...
  test("Auth Flow", async () => {
    // Signup
    const user = await CustomerAuthService.signup(storeId, {
      email,
    ...
    // Login
    const { sessionToken } = await CustomerAuthService.login(storeId, {
      email,
      password: "password123",
    });
  ...
  test("Order Access Control", async () => {
    ...
    // Valid Access
    const detail = await CustomerOrderService.getOrderDetail(
      storeId,
      customer.id,
      orderId,
    );
    ...
    await expect(
      CustomerOrderService.getOrderDetail(storeId, otherUserId, orderId),
    ).rejects.toThrow();
  });
});
