...
} from "../../apps/merchant-admin/src/lib/templates-registry";
import {
  recommendTemplate,
  RecommendationResult,
} from "../../apps/merchant-admin/src/lib/templates/recommendation-engine";
...
describe("Template Registry Integrity", () => {
  it("Hardening-01: Every category must have at least one template", () => {
    TEMPLATE_CATEGORIES.forEach((category) => {
      ...
      expect(
        templates.length,
        `Category ${category.slug} is empty`,
      ).toBeGreaterThan(0);
    });
  });
  ...
  it("Hardening-02: Every template in CANONICAL_TEMPLATES must belong to a valid category", () => {
    ...
    Object.values(CANONICAL_TEMPLATES).forEach((template) => {
      ...
      expect(
        categorySlugs.has(template.category),
  ...
  it("Hardening-03: Bidirectional mapping validity", () => {
    ...
    TEMPLATE_CATEGORIES.forEach((category) => {
      category.recommendedTemplates.forEach((templateId) => {
        const template =
          CANONICAL_TEMPLATES[templateId as keyof typeof CANONICAL_TEMPLATES];
        expect(
          template,
          `Category ${category.slug} recommends non-existent template ${templateId}`,
        ).toBeDefined();
        expect(
          template.category,
          `Template ${templateId} is mapped to ${template?.category} but recommended by ${category.slug}`,
        ).toBe(category.slug);
      });
    });
  });
...
describe("Recommendation Engine Snapshots", () => {
  ...
  const TEST_CASES = [
    ...
    { name: "Food Restaurant", state: createTestState("food-restaurant") },
    {
      ...
      state: createTestState("services-consulting"),
    },
