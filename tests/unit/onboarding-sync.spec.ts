...
describe('Onboarding Sync Contract (Unit)', () => {
    ...
    afterAll(async () => {
        ...
        if (storeId) {
            ...
            await prisma.storeProfile.deleteMany({ where: { storeId } }).catch(() => { });
            await prisma.bankBeneficiary.deleteMany({ where: { storeId } }).catch(() => { });
            await prisma.billingProfile.deleteMany({ where: { storeId } }).catch(() => { });
    ...
    it('QA-01: Maps & persists all fields correctly', async () => {
        ...
        const billing = await prisma.billingProfile.findUnique({ where: { storeId } });
        expect(billing?.legalName).toBe(mockState.business?.legalName);

        // Verify BankBeneficiary
        const bank = await prisma.bankBeneficiary.findFirst({ where: { storeId, isDefault: true } });
        expect(bank?.accountNumber).toBe(mockState.payments?.settlementBank?.accountNumber);
    ...
    it('QA-03: Test idempotency (Sync runs twice)', async () => {
        const initialState = await prisma.store.findUnique({ where: { id: storeId } });

        // Run sync again
        await syncOnboardingData(storeId, mockState);

        const finalState = await prisma.store.findUnique({ where: { id: storeId } });
        ...
        const banks = await prisma.bankBeneficiary.findMany({ where: { storeId, isDefault: true } });
