 ...
 * Create test order
 */
export async function createTestOrder(storeId: string, customerId?: string, overrides?: any) {
 ...
 */
export async function createTestInventory(merchantId: string, productId: string, variantId?: string, quantity = 100) {
...
export async function createCompleteTestStore(userId: string) {
    ...
    // Create products
    const { product: product1, variant: variant1 } = await createTestProduct(store.id, {
        title: 'Product 1',
        price: 5000,
    });

    const { product: product2, variant: variant2 } = await createTestProduct(store.id, {
        title: 'Product 2',
        price: 10000,
    });
    ...
    return {
        store,
        products: [{ product: product1, variant: variant1 }, { product: product2, variant: variant2 }],
