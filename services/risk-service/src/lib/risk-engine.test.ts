...
// Mock Prisma directly in factory
jest.mock('@vayva/db', () => ({
    prisma: {
...
describe('RiskEngine', () => {
    ...
    it('should calculate score for HIGH severity', async () => {
        (prisma.riskSignal.create as jest.Mock).mockResolvedValue({ id: '1', scoreDelta: 50 });
        (prisma.riskProfile.upsert as jest.Mock).mockResolvedValue({ merchantRiskScore: 50, status: 'NORMAL' });
        ...
        expect(prisma.riskSignal.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should trigger enforcement if score > 100', async () => {
        ...
        (prisma.riskProfile.upsert as jest.Mock).mockResolvedValue({ merchantRiskScore: 101, status: 'NORMAL' });
        (prisma.enforcementAction.create as jest.Mock).mockResolvedValue({ id: 'e1' });
