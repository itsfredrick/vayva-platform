...
export class TemplateSyncService {
    async syncCuratedPacks(): Promise<SyncResult> {
        const repoString = process.env.TEMPLATE_PACKS_REPO || 'vayva/vayva-template-packs';
        const [owner, repo] = repoString.split('/');

        const result: SyncResult = { source: 'curated', imported: 0, rejected: 0, errors: [] };
        ...
        try {
            ...
            if (!Array.isArray(packs)) throw new Error('Packs folder not found or is file');
            ...
            for (const pack of packs) {
                ...
                try {
                    ...
                    if ('content' in manifestData) {
                        const content = Buffer.from(manifestData.content, 'base64').toString();
    ...
    async syncGithubDiscovery(): Promise<SyncResult> {
        const result: SyncResult = { source: 'discovery', imported: 0, rejected: 0, errors: [] };
        ...
        try {
            ...
            for (const repo of search.items) {
                ...
                const existing = await prisma.template.findFirst({ where: { repoUrl: repo.html_url } });
