...
export const verifyMfaHandler = async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const opsUser = await prisma.opsUser.findUnique({ where: { id: decoded.sub } });
...
// Bootstrap tools
export const registerOpsHandler = async (req: FastifyRequest, reply: FastifyReply) => {
    const body = z.object({
        email: z.string().email(),
        password: z.string(),
        name: z.string(),
    ...
    const existing = await prisma.opsUser.findUnique({ where: { email: body.email } });
...
export const setupMfaHandler = async (req: FastifyRequest, reply: FastifyReply) => {
