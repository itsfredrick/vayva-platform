...
export const verifyPinHandler = async (
  ...
  if (!wallet || !wallet.pinHash)
    return reply.status(400).send({ error: "PIN not set" });
  ...
  if (
    ...
    return reply
      .status(403)
      .send({ error: "Wallet locked", lockedUntil: wallet.lockedUntil });
  }
  ...
  if (!isValid) {
    ...
    return reply
      .status(401)
      .send({ error: "Invalid PIN", attemptsRemaining: 5 - attempts });
  }
  ...
  return reply.send({
    status: "success",
    unlockToken: "v1_session_" + crypto.randomBytes(16).toString("hex"),
  });
};
...
export const createVirtualAccountHandler = async (
  ...
  if (IS_TEST_MODE) {
    const testVA = {
      ...
      vaProviderRef: "test_va_" + crypto.randomBytes(4).toString("hex"),
    };
  ...
  try {
    ...
    const response = await axios.post(
      ...
      {
        customer: store?.slug + "@vayva.ng", // Standardized to .ng
...
export const initiateWithdrawalHandler = async (
  ...
  const wallet = await prisma.wallet.findUnique({ where: { storeId } });
  if (!wallet || !wallet.pinHash)
    return reply.status(400).send({ error: "Wallet not ready or PIN not set" });
  ...
  if (wallet.availableKobo < amount)
  ...
  const withdrawal = await prisma.withdrawal.create({
    data: {
      ...
      referenceCode:
        "WD-" + crypto.randomBytes(6).toString("hex").toUpperCase(),
    },
  });
...
export const confirmWithdrawalHandler = async (
  ...
  if (withdrawal.status !== "PENDING_OTP")
    return reply.status(400).send({ error: "Invalid withdrawal state" });
  ...
  if (wallet?.kycStatus !== "VERIFIED") {
    return reply
      ...
      .send({
        error: "KYC_REQUIRED",
        message: "KYC must be VERIFIED to withdraw",
      });
  }

  // 2. Trigger Paystack Transfer (or Test Sim)
  if (IS_TEST_MODE) {
    // Simulated Success
    await prisma.$transaction([
      prisma.withdrawal.update({
        ...
        data: {
          status: "SUCCESS",
          providerRef: "test_tx_" + crypto.randomBytes(4).toString("hex"),
        },
      }),
